...
function receiptsOf(receipts, year, predicate) {
  ...
  return (receipts || []).filter((r) =>
    (r.datum || "").startsWith(y) && predicate(r)
  );
}
...
function _approxGST(brutto, year) { return _calcPreciseGST(brutto, year); }

// NEU: Feature 1 — ROI-Score
// Formel: (ersparnis / aufwand_faktor) × konfidenz_faktor × status_mod
const AUFWAND_FAKTOR = { niedrig: 1.0, mittel: 1.8, hoch: 3.5 };
...
function findOpportunities(profile, receipts, investments, year) {
  ...
  if (profile && typeof profile === "object" && (profile.tweaks !== undefined || profile.interviewAnswers !== undefined)) {
    const { tweaks = {}, receipts: r2 = [], investments: i2 = {}, interviewAnswers = {}, year: y2 } = profile;
    if (typeof buildUserProfile !== "function") return [{ __noData: true }];
    const p = buildUserProfile(interviewAnswers, tweaks);
    return findOpportunities(p, r2, i2, y2 || year);
  }

  if (!profile || !profile.beschaeftigung) return [{ __noData: true }];
  ...
  const {
    beschaeftigung, studium, einkunftsarten,
    familienstand, kinder, brutto,
    _ia: ia = {},
  } = profile;

  if (!brutto) return [{ __noData: true }];
    ...
    if (km > 0 && at > 0) {
      ...
      if (pendler > 0) {
        ops.push({
          ...
          belege_noetig: false, bereits_erfasst: false, prioritaet: 1, istGeschaetzt: false,
          detail_zahlen: { "Entfernung": km + " km", "Pendlertage": String(pendlertage), "Summe": fmtEUR(pendler) },
        });
      }
    }
  }
  ...
  if (profile.abzugsfaehigePositionen.homeoffice) {
    ...
    if (hoTage > 0) {
      ...
      ops.push({
        ...
        belege_noetig: false, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: false,
        detail_zahlen: { "HO-Tage": String(hoTage), "Anrechenbar": anrechenbar + " Tage", "Summe": fmtEUR(ho) },
      });
    }
  }
  ...
  if (studium) {
    const { typ, tageImBetrieb, tageBerufsschule, tageUni, kmZumBetrieb, kmZurBerufsschule, kmZurUni } = studium;
    ...
    if (kmZumBetrieb > 0 && tageImBetrieb > 0) {
      ...
      if (betriebPendler > 0) {
        ops.push({
          ...
          belege_noetig: false, bereits_erfasst: false, prioritaet: 1, istGeschaetzt: false,
          detail_zahlen: { "Betriebstage": String(tageImBetrieb), "km": String(kmZumBetrieb), "Summe": fmtEUR(betriebPendler) },
        });
      }
    }
    ...
    if (kmZurBerufsschule > 0 && tageBerufsschule > 0) {
      ...
      if (bsPendler > 0) {
        ops.push({
          ...
          beschreibung: `${kmZurBerufsschule} km × ${tageBerufsschule} BS-Tage = ${fmtEUR(bsPendler)}.` +
            (istAzubi ? " Beachte: Berufsschule kann erste Tätigkeitsstätte sein (BFH) → Pendlerpauschale, kein Reisekostenrecht." : " Beruflich veranlasst → Werbungskosten (§ 9 EStG)."),
          ...
          belege_noetig: false, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: false,
          detail_zahlen: { "BS-Tage": String(tageBerufsschule), "km": String(kmZurBerufsschule), "Summe": fmtEUR(bsPendler) },
        });
      }
    }
      ...
      if (uniPendler > 0) {
        if (typ === "erst") {
          ...
          ops.push({
            ...
            belege_noetig: false, bereits_erfasst: false, prioritaet: 3, istGeschaetzt: false,
            detail_zahlen: { "Uni-Tage": String(tageUni), "km": String(kmZurUni), "Fahrtkosten": fmtEUR(uniPendler), "Als SA anrechenbar": fmtEUR(saBetrag) },
          });
        } else {
          ops.push({
            ...
            belege_noetig: false, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: false,
            detail_zahlen: { "Uni-Tage": String(tageUni), "km": String(kmZurUni), "Summe": fmtEUR(uniPendler) },
          });
        }
      }
    }
    ...
    if (wkStudiBelege > 0) {
      ...
      ops.push({
        ...
        belege_noetig: true, bereits_erfasst: true, prioritaet: 3, istGeschaetzt: false,
      });
    }
    ...
    if (tageImBetrieb + tageBerufsschule + tageUni > 0) {
      ...
      ops.push({
        ...
        belege_noetig: false, bereits_erfasst: false, prioritaet: 9, istGeschaetzt: false,
      });
    }
  }
  ...
  if (hatAusbildung && studium) {
    const betriebPendler = (studium.kmZumBetrieb > 0 && studium.tageImBetrieb > 0)
      ? _pendler(studium.kmZumBetrieb, studium.tageImBetrieb, Kget) : 0;
    const bsPendler = (studium.kmZurBerufsschule > 0 && studium.tageBerufsschule > 0)
      ? _pendler(studium.kmZurBerufsschule, studium.tageBerufsschule, Kget) : 0;
    const hoTage = Number(ia.homeoffice_tage) || 0;
    ...
    const uniWK = (studium.typ !== "erst" && studium.kmZurUni > 0 && studium.tageUni > 0)
      ? _pendler(studium.kmZurUni, studium.tageUni, Kget) : 0;
  ...
  if (hatAusbildung || istAN) {
    const minijobGrenze = (
      window.TAX_CONFIG_RAW?.minijob_grenze_monatlich?.[String(yr)] ||
      Kget.minijob_grenze
    );
  ...
  if (istSelbst) {
    ...
    if (km > 0 && at > 0) {
      ...
      ops.push({
        ...
        belege_noetig: true, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: false,
        detail_zahlen: { "km (einfach)": String(km), "Tage": String(at), "Satz": "0,30 €/km", "Summe": fmtEUR(fahrtkostenBA) },
      });
    }
    const hoTage = Number(ia.homeoffice_tage) || 0;
    if (hoTage > 0) {
    ...
    if (baBelege > 0) {
      ops.push({
        ...
        belege_noetig: true, bereits_erfasst: true, prioritaet: 1, istGeschaetzt: false,
      });
    }
    ops.push({
      ...
      belege_noetig: false, bereits_erfasst: false, prioritaet: 8, istGeschaetzt: true,
    });
  }
  ...
  if (istRente) {
    ops.push({
      ...
      belege_noetig: true, bereits_erfasst: false, prioritaet: 1, istGeschaetzt: false,
    });
    ops.push({
      ...
      belege_noetig: false, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: true,
    });
  }
  ...
  if (istAN && !istStudent) {
    ...
    if (ueber > 0) {
      ops.push({
        ...
        belege_noetig: true, bereits_erfasst: wkBelege > 0, prioritaet: 1, istGeschaetzt: false,
        detail_zahlen: { "Belege": fmtEUR(wkBelege), "Pendler": fmtEUR(pendler), "Homeoffice": fmtEUR(hoSum), "Summe": fmtEUR(wkGesamt), "vs. Pauschale": "+ " + fmtEUR(ueber), "Grenzsteuersatz": Math.round(gst * 100) + " %" },
      });
    } else if (wkGesamt > 0) {
      ops.push({
        ...
        ersparnis: 0, aufwand: "niedrig", paragraph: "§ 9a EStG", anlage: "Anlage N",
        status: "info", aktion: "Keine Aktion nötig — Pauschale wird automatisch gewährt.",
        belege_noetig: false, bereits_erfasst: true, prioritaet: 9, istGeschaetzt: false,
      });
    }
    ...
    if (hat35aBelege && hatWKBelege) {
      const ermaessigung35a = r0(
        receiptsOf(receipts, yr, r => r.steuerkat === "haushaltsnahe")
          .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0) * 0.20
      );
  ...
  if (istAN && !istStudent && !!ia.fortbildung) {
    ...
    ops.push({
      ...
      aufwand: "niedrig", paragraph: "§ 9 Abs. 1 EStG", anlage: "Anlage N",
      status: hasBelege ? "lohnt" : "warn",
      aktion: hasBelege ? "Beträge in Anlage N eintragen." : "Belege im Scanner-Tab erfassen.",
      belege_noetig: true, bereits_erfasst: hasBelege, prioritaet: 2, istGeschaetzt: false,
    });
  }
  ...
  if (!!ia.gewerkschaft && (istAN || istAzubi)) {
    ...
    ops.push({
      ...
      belege_noetig: true, bereits_erfasst: false, prioritaet: 3, istGeschaetzt: false,
    });
  }
  ...
  if ((istAN || hatAusbildung) && kmFahrweg > 50) {
    ops.push({
      ...
      aufwand: "hoch", paragraph: "§ 9 Abs. 1 Satz 3 Nr. 5 EStG", anlage: "Anlage N",
      status: "info",
      aktion: "Voraussetzungen prüfen: eigener Hausstand + beruflich veranlasste Zweitwohnung.",
      belege_noetig: true, bereits_erfasst: false, prioritaet: 7, istGeschaetzt: true,
    });
  }
  ...
  if (!ia.riester && !(profile.riester_eigenanteil > 0) && !istSelbst && !istRente) {
    ops.push({
      ...
      aufwand: "hoch", paragraph: "§ 10a EStG", anlage: "Anlage AV",
      status: "info",
      aktion: "Riester-Vertrag beantragen; Zulagenantrag jährlich stellen.",
      belege_noetig: true, bereits_erfasst: false, prioritaet: 8, istGeschaetzt: true,
    });
  }
  ...
  if (!ia.bav && !(profile.bav_beitrag_jahres > 0) && profile.abzugsfaehigePositionen.bav) {
    ops.push({
      ...
      aufwand: "mittel", paragraph: "§ 3 Nr. 63 EStG", anlage: "—",
      status: "info",
      aktion: "Personalabteilung ansprechen; mind. 15 % AG-Zuschuss erfragen.",
      belege_noetig: false, bereits_erfasst: false, prioritaet: 5, istGeschaetzt: true,
    });
  }
  ...
  if (!!ia.spenden) {
    ...
    ops.push({
      ...
      beschreibung: hasBelege
        ? `${fmtEUR(spendenSum)} erfasst — als Sonderausgaben absetzbar.`
        : "Spenden angegeben, aber keine Quittungen erfasst. Bis 20 % des Einkommens absetzbar.",
      ersparnis: hasBelege ? r0(spendenSum * gst) : 0,
      aufwand: "niedrig", paragraph: "§ 10b EStG", anlage: "Mantelbogen",
      status: "warn",
      aktion: "Zuwendungsbestätigungen anfordern (> 300 €) — Kontoauszug reicht darunter.",
      belege_noetig: true, bereits_erfasst: hasBelege, prioritaet: 3, istGeschaetzt: false,
    });
  }
  ...
  ops.push({
    ...
    ersparnis: 0,
    aufwand: "niedrig", paragraph: "§ 10 Abs. 1 Nr. 3 EStG", anlage: "Anlage Vorsorgeaufwand",
    status: "info",
    aktion: "Lohnsteuerbescheinigung hochladen — Werte werden meist automatisch übernommen.",
    belege_noetig: false, bereits_erfasst: true, prioritaet: 10, istGeschaetzt: true,
  });
  ...
  if (profile.kirchenmitglied && einkunftsarten?.kapitalertraege) {
    ops.push({
      ...
      belege_noetig: true, bereits_erfasst: false, prioritaet: 3, istGeschaetzt: true,
      detail_zahlen: {},
    });
  }
  ...
  if (profile.unterhalt_betrag > 0) {
    ...
    ops.push({
      ...
      belege_noetig: true, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: false,
      detail_zahlen: { "Gezahlt": fmtEUR(profile.unterhalt_betrag), "Max. absetzbar": fmtEUR(max_absetzbar), "Ersparnis ca.": fmtEUR(ersparnis_unterhalt) },
    });
  }
  ...
  if (profile.kirchenmitglied && profile.bundesland && profile.bundesland !== "default" && brutto) {
    ...
    const estGrob  = typeof calcESt === "function"
      ? calcESt(Math.max(0, brutto * 0.72 - _wkP), String(yr))
      : 0;
    ...
    if (kistGrob > 0) {
      ops.push({
        ...
        belege_noetig: true, bereits_erfasst: false, prioritaet: 2, istGeschaetzt: true,
        detail_zahlen: { "Bundesland": profile.bundesland, "KiSt-Satz": (kistSatz * 100) + " %", "Kirchensteuer ca.": fmtEUR(kistGrob) },
      });
    }
  }
  ...
  if (!!ia.kapitalertraege) {
    ...
    ops.push({
      ...
      aufwand: "niedrig", paragraph: "§ 44a EStG", anlage: "—",
      status: "action",
  ...
  if (verluste.length > 0) {
    ...
    ops.push({
      ...
      aufwand: "mittel", paragraph: "§ 20 Abs. 6 EStG", anlage: "Anlage KAP",
      status: "lohnt",
  ...
  if (!!ia.kapitalertraege && brutto < Kget.grundfreibetrag + 5000) {
    ops.push({
      ...
      aufwand: "niedrig", paragraph: "§ 32d Abs. 6 EStG", anlage: "Anlage KAP",
      status: "lohnt",
  ...
  if (dlSum > 0) {
    ...
    if (dlAusgeschoepft) {
      ops.push({
        ...
        ersparnis: maxDlErm,
        aufwand: "niedrig", paragraph: "§ 35a Abs. 2 EStG", anlage: "Mantelbogen",
        status: "lohnt",
        aktion: `Neue DL-Verträge erst ab Januar ${yr + 1} abrechnen lassen.`,
        belege_noetig: false, bereits_erfasst: false, prioritaet: 1, istGeschaetzt: true,
      });
      ...
      ops.push({
        ...
        aufwand: "mittel", paragraph: "§ 35a Abs. 2 EStG", anlage: "Mantelbogen",
        status: "lohnt",
  ...
  if (hwkSum > 0) {
    ...
    if (hwkAusgeschoepft) {
      ops.push({
        ...
        ersparnis: maxHwkErm,
        aufwand: "niedrig", paragraph: "§ 35a Abs. 3 EStG", anlage: "Mantelbogen",
        status: "lohnt",
      ...
      ops.push({
        ...
        aufwand: "mittel", paragraph: "§ 35a Abs. 3 EStG", anlage: "Mantelbogen",
        status: "lohnt",
  ...
  if (!!ia.krankenkosten) {
    ...
    const zumutbar = typeof calcZumutbareEigenbelastung === "function"
      ? calcZumutbareEigenbelastung(brutto, familienstand, kinderAnzahl)
      : r0(brutto * 0.04);
    ...
    ops.push({
      ...
      beschreibung: ueberzumutbar > 0
        ? `${fmtEUR(abSum)} − zumutbare Eigenbelastung ${fmtEUR(zumutbar)} (§ 33 Abs. 3 EStG, gestaffelt) = ${fmtEUR(ueberzumutbar)} absetzbar.`
        : `${fmtEUR(abSum)} liegen unter zumutbarer Eigenbelastung (${fmtEUR(zumutbar)}) — kein Abzug möglich.`,
      ersparnis: r0(ueberzumutbar * gst),
      aufwand: "mittel", paragraph: "§ 33 EStG", anlage: "Mantelbogen",
      status: ueberzumutbar > 0 ? "lohnt" : "info",
      aktion: "Rezepte, Zuzahlungen, Brillen, Arztfahrten sammeln.",
      belege_noetig: true, bereits_erfasst: abBelege.length > 0,
      prioritaet: ueberzumutbar > 0 ? 2 : 8, istGeschaetzt: false,
      detail_zahlen: { "AB-Belege": fmtEUR(abSum), "Zumutbare Belastung": fmtEUR(zumutbar), "Abziehbar": fmtEUR(ueberzumutbar) },
    });
  }

  if (!!ia.behinderung) {
    // §33b EStG — Pauschbeträge nach GdB (Stand 2021+)
    const _GDB_PAUSCH = { 20: 384, 30: 620, 50: 1140, 70: 1780, 80: 2120, 100: 2840 };
    ...
    ops.push({
      ...
      aufwand: "niedrig", paragraph: "§ 33b EStG", anlage: "Mantelbogen",
      status: "action",
  ...
  if (verheiratet && steuerklasse === 4) {
    ops.push({
      ...
      ersparnis: 0,
      aufwand: "mittel", paragraph: "§ 38b EStG", anlage: "—",
      status: "info",
      aktion: "Wechselantrag beim Finanzamt — einmal pro Jahr möglich.",
      belege_noetig: false, bereits_erfasst: false, prioritaet: 7, istGeschaetzt: true,
    });
  }
  ...
  if (istAN || istAzubi || istSelbst) {
    ...
    const hatTelBeleg  = receiptsOf(receipts, yr, r =>
      r.steuerkat === "werbungskosten" &&
      ["telefon", "internet", "handy"].some(k => (r.haendler || "").toLowerCase().includes(k))
    ).length > 0;
  ...
  if (istAN || istAzubi) {
    ops.push({
      ...
      prioritaet: 4,
      istGeschaetzt: true,
      detail_zahlen: { "Typischer Preis": "~35 €", "Ersparnis ca.": fmtEUR(r0(35 * gst)) },
    });
  }
  ...
  if (verheiratet && (ia.steuerklasse === "3" || ia.steuerklasse === 3 ||
      ia.steuerklasse === "5" || ia.steuerklasse === 5)) {
    ops.push({
      id: "faktor_verfahren",
      kategorie: "sonderausgaben",
  ...
  const scoredOps = ops
    .filter((o) => (o.ersparnis || 0) > 0 || o.status === "warn" || o.status === "action" || o.status === "info")
    .map((op) => ({ ...op, roi_score: calcROIScore(op) }));
...
const AUFWAND_LABEL = { niedrig: "leicht", mittel: "mittel", hoch: "aufwändig" };

function OpportunityCard({ op, expanded, onToggle }) {
  ...
  return (
    ...
    >
      <div className="opt-card-top">
        <span className={`opt-badge ${op.status}`}>{op.status}</span>
        {op.roi_score > 0 && (
          <span
            className="opt-roi-badge"
            title="ROI-Score: Ersparnis ÷ Aufwand × Konfidenz"
            style={{
        ...
        {/* NEU: Feature D — Schätzungs-Badge */}
        {op.istGeschaetzt && (
          <span
            style={{
            ...
            title="Geschätzte Ersparnis — kein konkreter Beleg vorhanden"
          >
            ~Schätzung
          </span>
        )}
        <span className="opt-para">{op.paragraph}</span>
        <span className={`opt-chip-aufwand ${op.aufwand}`}>{AUFWAND_LABEL[op.aufwand] || op.aufwand}</span>
      </div>
      <div className="opt-card-row">
        <div className="opt-titel">
          {op.titel}
          {op.bereits_erfasst && op.status !== "warn" && (
            <span className="opt-done-mark"> · ✓ Belege da</span>
          )}
        </div>
        <div className={`opt-ersparnis ${(op.ersparnis || 0) === 0 ? "muted" : ""}`}>
          {ersparnisStr}
        </div>
      </div>
      <div className="opt-beschreibung">{op.beschreibung}</div>

      <div className="opt-detail">
        <div className="opt-detail-grid">
          <div className="opt-detail-label">Anlage</div>
          <div className="opt-detail-value">{op.anlage || "—"}</div>
          <div className="opt-detail-label">Paragraph</div>
          <div className="opt-detail-value">{op.paragraph}</div>
          <div className="opt-detail-label">Belege nötig</div>
          <div className="opt-detail-value">{op.belege_noetig ? "ja" : "nein"}</div>
          <div className="opt-detail-label">Bereits erfasst</div>
          <div className="opt-detail-value">{op.bereits_erfasst ? "ja" : "nein"}</div>
          {op.detail_zahlen && Object.entries(op.detail_zahlen).map(([k, v]) => (
            <React.Fragment key={k}>
              <div className="opt-detail-label">{k}</div>
              <div className="opt-detail-value">{v}</div>
            </React.Fragment>
          ))}
        </div>
        <div className="opt-aktion">→ {op.aktion}</div>
        <div className="opt-disclaimer">{DISCLAIMER}</div>
      </div>
    </div>
  );
}
...
function TaxOptimizer({
  ...
  const belegeFehlen = ops.filter(
    (o) => !o.bereits_erfasst && (o.status === "action" || o.status === "warn") && o.belege_noetig
  ).length;
  ...
  return (
    <React.Fragment>
      <div className="tax-modal-backdrop" onClick={onClose}>
        <div className="tax-modal opt-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
          <div className="opt-header">
            <div className="opt-header-title">
              Steuer-Optimierer · {yr}
              <div style={{ fontSize: "11px", color: "var(--text-faint)", marginTop: "2px" }}>
                Sortiert nach ROI-Score (Ersparnis ÷ Aufwand)
              </div>
            </div>
            {(gesamtEcht > 0 || gesamtGeschaetzt > 0) && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
                {gesamtEcht > 0 && (
                  <span className="opt-header-pot">+ {fmtEUR(gesamtEcht)}</span>
                )}
                {gesamtGeschaetzt > 0 && (
                  <span style={{ fontSize: 11, color: "var(--text-faint)", fontVariantNumeric: "tabular-nums" }}>
                    + {fmtEUR(gesamtGeschaetzt)} (Schätzung)
                  </span>
                )}
              </div>
            )}
            <button className="opt-header-close" onClick={onClose} aria-label="Schließen">
              {typeof Icon !== "undefined" && Icon.Close
                ? <Icon.Close />
                : (
          ...
          <div className="opt-metrics">
            <div className="opt-metric">
              <div className="opt-metric-label">Gesichert</div>
              <div className="opt-metric-value success">{fmtEUR(gesamtEcht)}</div>
            </div>
            <div className="opt-metric">
              <div className="opt-metric-label">Sofort umsetzbar</div>
              <div className="opt-metric-value action">{sofort}</div>
            </div>
            <div className="opt-metric">
              <div className="opt-metric-label">Belege fehlen</div>
              <div className="opt-metric-value warn">{belegeFehlen}</div>
            </div>
          </div>

          <div className="opt-filters" role="tablist">
            {/* NEU: Feature D — Gesichert-Filter als erster Tab */}
            <button
            ...
            >
              ✓ Gesichert ({ops.filter(o => !o.istGeschaetzt && (o.ersparnis || 0) > 0).length})
            </button>
            {kategorien.map((k) => (
              <button
                key={k}
              ...
              >
                {k === "alle" ? `Alle (${ops.length})` : `${KATEGORIE_LABELS[k] || k} (${ops.filter((o) => o.kategorie === k).length})`}
              </button>
            ))}
          </div>
          ...
          <div className="opt-list">
            {noData ? (
              <div className="opt-empty">
                <div className="opt-empty-emoji">👤</div>
                <strong style={{ display: "block", marginBottom: 8, color: "var(--text)" }}>
                  Persönliche Daten fehlen
                </strong>
                Ohne dein Jahresbruttogehalt kann der Optimierer keine realistischen
                Steuerersparnisse berechnen. Bitte fülle zuerst deine persönlichen Daten aus —
                über den{" "}
                ...
                {" "}Button oben rechts im Steuer-Tab.
              </div>
            ) : visible.length === 0 ? (
              <div className="opt-empty">
                <div className="opt-empty-emoji">🎯</div>
                Keine offenen Optimierungen in dieser Kategorie.
                Wechsle den Filter oder erfasse mehr Daten in deinen persönlichen Daten.
              </div>
            ) : visible.map((op) => (
              <OpportunityCard
                key={op.id}
...
function OptimizerFAB({ count = 0, onClick }) {
  return (
    <React.Fragment>
      <button
        className={`opt-fab ${count > 0 ? "pulse" : ""}`}
        onClick={onClick}
        aria-label="Steuer-Optimierer öffnen"
        title={count > 0 ? `${count} Optimierungs-Chance${count > 1 ? "n" : ""}` : "Steuer-Optimierer"}
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          {/* Lupe + Glühbirne kombiniert: vereinfachtes Target/Idee-Glyph */}
        ...
        {count > 0 && <span className="opt-fab-badge">{count > 99 ? "99+" : count}</span>}
      </button>
    </React.Fragment>
  );
}
