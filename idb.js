  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (row._enc && typeof window.secureImageDecrypt === "function") {
        try {
          dataUrl = await window.secureImageDecrypt(dataUrl);
        } catch {
          continue;
        } // Entschlüsselung fehlgeschlagen → Bild überspringen
      }
      map.set(row.id, {
        dataUrl,
...
function _invalidate(id, newValue) {
  if (newValue === undefined) _imgCache.delete(id);else _imgCache.set(id, newValue);
  _emit(id);
}
...
function useReceiptImage(id) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      if (s) {
        s.delete(fn);
        if (s.size === 0) _imgListeners.delete(id);
      }
    };
  }, [id]);
  return val;
