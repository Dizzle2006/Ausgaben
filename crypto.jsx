...
async function _deriveKey(pin, salt) {
  ...
  return crypto.subtle.deriveKey(
    { name: "PBKDF2", salt, iterations: 600_000, hash: "SHA-256" }, // OWASP 2024: ≥600k Iter.
    raw,
...
async function _enc(key, text) {
  ...
  return JSON.stringify({ _v: 1, iv: _b64(iv), ct: _b64(new Uint8Array(ct)) });
}
...
function _readFromStorage(store, key) {
  try {
    ...
    const expected = _stateChecksum({ fails: s.fails || 0, lockUntil: s.lockUntil || 0 });
    if (s._cs !== expected) return { fails: _MAX_ATTEMPTS, lockUntil: 0, __tampered: true };
    return s;
  } catch { return null; }
}
...
function _readLockState() {
  ...
  if (!ls && !ss) return { fails: 0, lockUntil: 0 };
  ...
  if (!ls || !ss) {
    ...
    return { fails: _MAX_ATTEMPTS, lockUntil: 0, __missing: true };
  }
  ...
  if (ls.__tampered || ss.__tampered) {
    ...
    return { fails: _MAX_ATTEMPTS, lockUntil: 0 };
  }
...
function _saveLockState(s) {
  const toStore = { ...s, _cs: _stateChecksum({ fails: s.fails || 0, lockUntil: s.lockUntil || 0 }) };
  const json = JSON.stringify(toStore);
  try { localStorage.setItem(_LOCK_STATE_KEY, json); } catch {/* quota */}
  try { sessionStorage.setItem(_LOCK_STATE_KEY, json); } catch {/* privat mode */}
}

function _clearLockState() {
  // Fehlerzähler auf 0 zurücksetzen — in BEIDEN Storages mit gültiger Prüfsumme.
  const clean = { fails: 0, lockUntil: 0 };
...
function _checkLock() {
  ...
  if (s.__missing) {
    ...
    _saveLockState({ fails: _MAX_ATTEMPTS, lockUntil });
    return { locked: true, msRemaining: _BASE_LOCK_MS, fails: _MAX_ATTEMPTS };
  }
  if (!s.lockUntil) return { locked: false, msRemaining: 0, fails: s.fails };
  const remaining = s.lockUntil - Date.now();
  if (remaining <= 0) {
    const next = { ...s, lockUntil: 0 };
    _saveLockState(next);
    return { locked: false, msRemaining: 0, fails: s.fails };
  }
  return { locked: true, msRemaining: remaining, fails: s.fails };
}
...
function _recordFail() {
  ...
  const newState  = { fails: newFails, lockUntil: s.lockUntil || 0 };
...
async function _unlock(pin) {
  ...
  if (lock.locked) {
    ...
    return { ok: false, locked: true, msRemaining: lock.msRemaining,
    ...
    try {
      ...
      if (result !== _VERIFY_TXT) {
        ...
        if (s.lockUntil > Date.now()) {
          ...
          return { ok: false, locked: true, msRemaining: s.lockUntil - Date.now(),
            err: `Falscher PIN. Tresor gesperrt für ${sek} Sekunden.` };
        }
        const versucheText = remaining > 0
          ? ` Noch ${remaining} Versuch${remaining === 1 ? "" : "e"}.`
          : "";
        return { ok: false, err: `Falscher PIN — bitte erneut versuchen.${versucheText}` };
      }
    } catch {
      const s = _recordFail();
      const remaining = _MAX_ATTEMPTS - s.fails;
...
window.secureChangePin = async function secureChangePin(oldPin, newPin) {
  try {
    if (!oldPin || !newPin) return { ok: false, err: "PIN darf nicht leer sein." };
    if (String(newPin).length < 6) return { ok: false, err: "Neuer PIN benötigt mindestens 6 Zeichen." };
    if (String(oldPin) === String(newPin)) return { ok: false, err: "Neuer PIN muss sich vom alten unterscheiden." };

    const verifyBlob = localStorage.getItem(_VERIFY_KEY);
    if (!verifyBlob) return { ok: false, err: "Kein PIN gesetzt — Reset hier nicht möglich." };

    // 1) Alten Schlüssel mit aktuellem Salt prüfen
    const oldSaltStr = localStorage.getItem(_SALT_KEY);
    if (!oldSaltStr) return { ok: false, err: "Salt fehlt — Datenintegrität gestört." };
    ...
    try {
      ...
      return { ok: false, err: "Aktueller PIN ist falsch." };
    }
    ...
    let plainData = "{}";
    if (dataBlob && _isEncrypted(dataBlob)) {
      try { plainData = await _dec(oldKey, dataBlob); }
      catch { return { ok: false, err: "Daten konnten nicht entschlüsselt werden." }; }
    }
    ...
    _activeKey = newKey;
    try { localStorage.removeItem(_LOCK_STATE_KEY); } catch {}
    try { sessionStorage.removeItem(_LOCK_STATE_KEY); } catch {}

    return { ok: true };
  } catch (e) {
    console.error("[crypto] secureChangePin:", e);
    return { ok: false, err: "Unbekannter Fehler beim PIN-Wechsel." };
  }
};
...
window.secureGetEncryptionInfo = function secureGetEncryptionInfo() {
  ...
  return {
    ...
    ivBytes: 12,
    pinSet: verifySet,
    keyResident: !!_activeKey, // Schlüssel nur im RAM, nie im Storage
...
window.secureChatEncrypt = async (text) => {
  ...
  try { return await _enc(_activeKey, text); } catch { return text; }
};

window.secureChatDecrypt = async (blob) => {
  if (!_activeKey || !_isEncrypted(blob)) return blob;
  try { return await _dec(_activeKey, blob); } catch { return blob; }
};
...
function PinGate({ children }) {
  ...
  React.useEffect(() => {
    const lock = _checkLock();
    if (lock.locked) _startCountdown(lock.msRemaining);
    else setTimeout(() => pinRef.current?.focus(), 80);
  }, []);
  ...
  async function handleSubmit(e) {
    ...
    const p = pin.trim();
    if (p.length < 6) { setError("Mindestens 6 Zeichen."); return; }
    if (phase === "setup") {
      if (p !== confirm.trim()) { setError("PINs stimmen nicht überein."); return; }
    }
    setBusy(true);
    setError("");
    try {
    ...
    <div className="pin-gate">
      <div className="pin-card">
        ...
        <div className="pin-title">
          {isLocked ? "Tresor gesperrt" : isSetup ? "PIN setzen" : "App entsperren"}
        </div>
        <div className="pin-sub">
          {isLocked
            ? "Zu viele Fehlversuche. Bitte kurz warten."
            : isSetup
            ? "Wähle einen PIN — deine Steuerdaten werden damit verschlüsselt gespeichert."
            : "Gib deinen PIN ein, um auf deine verschlüsselten Steuerdaten zuzugreifen."}
        </div>
        ...
        {isLocked && (
          <div className="pin-lockout">
            Entsperrt in
            <span className="pin-lockout-timer">{secLeft}s</span>
          </div>
        )}

        {/* Formular (nur wenn nicht gesperrt) */}
        {!isLocked && (
          <form onSubmit={handleSubmit} style={{ width: "100%" }} autoComplete="off">
            <div className="pin-label">{isSetup ? "Neuer PIN" : "PIN"}</div>
            <input
              ref={pinRef}
              className="pin-input"
              type="password"
            ...
            {isSetup && (
              <React.Fragment>
                <div className="pin-label">PIN bestätigen</div>
                <input
                  ref={confirmRef}
                  className="pin-input"
                  type="password"
            ...
            >
              {busy ? "Wird verarbeitet…" : isSetup ? "Tresor einrichten" : "Entsperren"}
            </button>
          </form>
        )}

        {error && !isLocked && <div className="pin-error">{error}</div>}

        <div className="pin-hint">
          {isSetup
            ? "Bei vergessenem PIN bleiben alle Belege erhalten — nur die Interview-Antworten müssen neu eingegeben werden."
            : "Vergessen? Alle Daten außer den Interview-Antworten sind weiterhin zugänglich."}
        </div>
...
(function _secureExport() {
  try {
    Object.defineProperty(window, "PinGate", {
      value: PinGate, writable: false, configurable: false, enumerable: true,
    });
  } catch { window.PinGate = PinGate; }
})();
