...
async function saveBedarfsanalyse(profil) {
  try {
    ...
    const blob = typeof window.secureChatEncrypt === "function" ? await window.secureChatEncrypt(json) : json;
...
function _openBudgetChatsDb() {
  ...
  _budgetChatsDbPromise = new Promise((resolve, reject) => {
    if (typeof indexedDB === "undefined") {
      reject(new Error("IndexedDB n/a"));
      return;
    }
    const req = indexedDB.open(BUDGET_CHATS_DB, BUDGET_CHATS_VER);
    req.onupgradeneeded = e => {
      const db = e.target.result;
      if (!db.objectStoreNames.contains(BUDGET_CHATS_STORE)) db.createObjectStore(BUDGET_CHATS_STORE, {
        keyPath: "id"
      });
    };
    req.onsuccess = () => resolve(req.result);
    req.onerror = () => {
      _budgetChatsDbPromise = null;
      reject(req.error);
    };
  });
  ...
  try {
    ...
    for (const c of all) {
      if (c._msgEnc && typeof c.messages === "string" && typeof window.secureChatDecrypt === "function") {
        try {
          c.messages = JSON.parse(await window.secureChatDecrypt(c.messages));
        } catch {
          c.messages = [];
        }
...
function _getMonthTotals(monthData) {
  ...
  const variable = (monthData.variable || []).reduce((s, cat) => s + (cat.entries || []).reduce((cs, e) => cs + (Number(e.amount) || 0), 0), 0);
  return {
    income,
    fixed,
    savings,
    variable,
    total: fixed + savings + variable
  };
}
...
function computeBudgetAlerts(state, profil) {
  ...
  if (income > 0 && remaining < 0) {
    alerts.push({
      id: "budget_ueberschritten",
      prio: 1,
      farbe: "var(--negative, #e03)",
      icon: "🚨",
  ...
  for (const cat of monthData.variable || []) {
    ...
    if (pct >= 1.1) {
      alerts.push({
        id: `kat_ueber_${cat.id}`,
        prio: 2,
        farbe: "var(--warning, #f80)",
        icon: "📊",
...
const _BUDGET_INTENTS = [{
  key: "hilfe",
  re: /^(hilfe|help|\?+$|was kannst|themen|übersicht|wie funktioniert|womit kann|fangen wir an|los geht|starten|start|zeig.*optionen)/i
}, {
  key: "budget_check",
  re: /budget.?(analyse|check|übersicht|status|wie\s+stehe|wie\s+lieg)|mein.*budget|budget.*monat|wie.*ausgaben|ausgaben.*überblick|was.*ausgegeben|monatsbilanz|monat.*analyse/i
}, {
  key: "sparziel",
  re: /sparz(iel|rate|en)|wie\s+viel.*spar|spar.*ziel|spare.*monat|monat.*spar|rücklage.*spar|zurücklegen|spare.*zu\s+wenig|zu\s+wenig.*spar/i
}, {
  key: "fixkosten",
  re: /fixkosten|fixe.*kosten|feste.*kosten|monatliche.*kosten|abo.*fix|miete.*fix|versicherung.*monat|fix.*senken|fix.*reduzier/i
}, {
  key: "kategorie",
  re: /kategorie|rubrik|bereich|wofür.*geld|wo.*ausgib|wohin.*geld|welche.*kategorie|am\s+meisten.*ausgib|meiste.*geld/i
}, {
  key: "restaurant",
  re: /restaurant|essen.*gehen|auswärts.*essen|gastronomie|food.*delivery|lieferando|uber\s*eats|takeaway|pizza\s+bestell/i
}, {
  key: "einkaufen",
  re: /einkauf|supermarkt|lebensmittel|aldi|lidl|rewe|edeka|wocheneinkauf/i
}, {
  key: "abo",
  re: /\babo(s)?\b|abonnement|subscription|netflix|spotify|amazon\s+prime|disney|streaming|wiederkehr/i
}, {
  key: "transport",
  re: /transport|tanken|benzin|parkplatz|öpnv|fahrtkosten|auto.*kosten/i
}, {
  key: "freizeit",
  re: /freizeit|hobby|sport|unterhaltung|kino|konzert|ausgehen|entertainment/i
}, {
  key: "notgroschen",
  ...
  key: "muster",
  re: /muster|trend|entwicklung|immer.*zu\s+viel|regelmäßig.*überschreit|monat.*vergleich/i
}, {
  key: "optimierung",
  re: /optimier|wo.*sparen|tipps?.*budget|budget.*tipp|was.*reduzier|kosten.*senken|einsparen|haushalt.*optimier/i
}, {
  key: "forecast",
  re: /prognose|hochrechnung|bis\s+ende.*monat|monatsende|wie\s+viel.*noch|noch\s+wie\s+viel|forecast/i
}, {
  key: "bedarfsanalyse",
  re: /bedarfsanalyse|finanzprofil|profil.*einrichten|profil.*ändern|profil.*aktualisier|meine.*daten\s*ändern|einkommen.*ändern/i
}, {
  key: "ueberschuss",
  re: /überschuss|übrig\s+bleibt|was\s+mach.*restl|restl.*geld|geld\s+übrig|was\s+tun.*plus/i
}, {
  key: "schulden",
  re: /schulden|kredit|dispo|abzahlen|schulden.*tilgen|tilgung/i
}, {
  key: "versicherung",
  re: /versicherung|haftpflicht|hausrat|kfz.*versicherung|versicherung.*lohnt/i
}];
...
- Finanzielle Ziele: ${profil.finanzielleZiele?.join(", ") || "keine angegeben"}`;
  ...
  if (totals) {
    const catList = (cats || []).map(c => `  • ${c.label}: ${c.spent.toFixed(2)} € ausgegeben (Budget: ${c.budget.toFixed(2)} €, ${c.budget > 0 ? (c.spent / c.budget * 100).toFixed(0) + "%" : "–"})`).join("\n");
...
function deterministicBudgetAntwort(input, ctx) {
  ...
  const noData = (what = "Budgetdaten") => `Ich konnte keine ${what} für ${monat} finden. Trage zuerst Einkommen und Ausgaben ein, dann kann ich dir konkrete Analysen liefern.`;
  switch (intent) {
    case "hilfe":
      return `**BudgetBot – Dein Finanzcoach**
    ...
    case "budget_check":
      {
        ...
        const catLines = cats.length > 0 ? cats.map(c => `  ${c.label}: ${fmtEUR(c.spent)}${c.budget > 0 ? ` / ${fmtEUR(c.budget)} (${c.pct ? Math.round(c.pct * 100) : 0}%)` : ""}`).join("\n") : "  (keine variablen Kategorien erfasst)";
        let bewertung = "";
        if (income === 0) bewertung = "⚠️ Kein Einkommen erfasst – Bilanz unvollständig.";else if (remaining < 0) bewertung = `🚨 Budget überschritten um ${fmtEUR(Math.abs(remaining))}.`;else if (spentPct > 90) bewertung = `⚠️ Du hast ${spentPct}% deines Einkommens verplant – wenig Puffer.`;else if (spentPct < 70) bewertung = `✅ Du bist gut dabei – ${spentPct}% verplant, ${fmtEUR(remaining)} frei.`;else bewertung = `👍 Solide: ${spentPct}% verplant, ${fmtEUR(remaining)} übrig.`;
    ...
    case "fixkosten":
      {
        ...
        const bewertung = fixPct === 0 ? "Keine Fixkosten erfasst." : fixPct <= 35 ? `✅ Fixkosten niedrig (${fixPct}%) – guter Handlungsspielraum.` : fixPct <= 50 ? `👍 Fixkosten moderat (${fixPct}%) – noch im Rahmen.` : `⚠️ Fixkosten hoch (${fixPct}%) – über 50% gelten als kritisch.`;
...
**Faustregel:** Pro Person ca. 200–300 €/Monat für Lebensmittel.`;
    ...
    case "abo":
      return `**Abonnements & wiederkehrende Kosten**
...
${diff < 0 ? `🚨 Auf Kurs, Budget um ${fmtEUR(Math.abs(diff))} zu überschreiten.` : diff < income * 0.1 ? `⚠️ Knapper Puffer – wenig Spielraum für unerwartete Ausgaben.` : `✅ Prognostiziertes Plus von ${fmtEUR(diff)} – du liegst gut im Plan.`}

${daysLeft > 0 ? `Noch ${daysLeft} Tage – Tageslimit für Budget-Einhaltung: ${fmtEUR(Math.max(0, (income - total) / daysLeft))}/Tag.` : ""}`;
      }
    case "bedarfsanalyse":
      return profil ? `**Dein Finanzprofil**
...
Faustregel: Mindestens 50% des Überschusses sparen/investieren, 50% genießen.`;
    ...
    case "schulden":
      return `**Schulden abbauen – Strategie**
...
**Kreditkarten-Schulden:** Zinsen oft 18–24% p.a. – Prio 1.`;
    case "versicherung":
      return `**Versicherungen – was ist sinnvoll?**
...
**Tipp:** Versicherungsvergleich jährlich – besonders für KFZ lohnt sich Wechseln.`;
    default:
      return `Ich habe deine Frage nicht ganz verstanden. Tippe **"Hilfe"** für eine Übersicht meiner Themen, oder formuliere deine Frage neu.`;
  }
}
...
function _renderBudgetMd(text) {
  ...
  function inlineRender(str) {
    ...
    while ((m = re.exec(str)) !== null) {
      ...
      if (m[2]) parts.push(React.createElement("strong", {
        key: m.index
      }, m[2]));else if (m[3]) parts.push(React.createElement("code", {
        key: m.index,
        style: {
...
function BudgetMsgBubble({
  ...
  }, isBot ? _renderBudgetMd(content) : content));
}
...
function BudgetSuggestions({
  ...
  }, "Frag mich zum Beispiel:"), /*#__PURE__*/React.createElement("div", {
...
function BedarfsanalyseWizard({
  ...
  const patch = (key, val) => setForm(f => ({
    ...f,
    [key]: val
  }));
  const sparZielAbsolut = form.sparZielTyp === "prozent" ? Math.round(form.nettoEinkommen * form.sparZielProzent / 100) : form.sparZielMonatlich;
  const SCHRITTE = [{
    titel: "Einkommen",
    icon: "💶"
  }, {
  ...
  const addZiel = () => {
    ...
    patch("finanzielleZiele", [...form.finanzielleZiele, {
      id: String(Date.now()),
      label: neuesZiel.label,
      betrag: Number(neuesZiel.betrag) || 0,
      bisDatum: neuesZiel.bisDatum || null
    }]);
    setNeuesZiel({
      label: "",
      betrag: "",
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Finanzprofil"), /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-sub"
  }, "Schritt ", step + 1, " von ", SCHRITTE.length)), /*#__PURE__*/React.createElement("div", {
    style: {
      width: 60
    }
  })), /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      width: `${(step + 1) / SCHRITTE.length * 100}%`,
  ...
  }, SCHRITTE.map((s, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
  ...
  }, s.icon, /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10
    }
  }, s.titel)))), step === 0 && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      marginBottom: 8
    }
  ...
  }, "Dein Netto-Einkommen"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: "var(--text-muted)",
      marginBottom: 18,
  ...
  }, "Dein monatliches Nettogehalt nach Steuern und Abz\xFCgen \u2013 die Grundlage aller Berechnungen."), /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Monatliches Nettoeinkommen (\u20AC)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "50",
    ...
    style: inputStyle
  }), form.nettoEinkommen > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      padding: "10px 14px",
      background: "var(--surface-2)",
  ...
  }, "Nach 50/30/20: Bed\xFCrfnisse ", fmtEUR(form.nettoEinkommen * 0.5), " \xB7 W\xFCnsche ", fmtEUR(form.nettoEinkommen * 0.3), " \xB7 Sparen ", fmtEUR(form.nettoEinkommen * 0.2))), step === 1 && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      marginBottom: 8
    }
  ...
  }, "Dein Sparziel"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: "var(--text-muted)",
      marginBottom: 18,
  ...
  }, "Wie viel m\xF6chtest du jeden Monat auf die Seite legen?"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      marginBottom: 16
    }
  }, [["betrag", "Fester Betrag (€)"], ["prozent", "Prozentsatz (%)"]].map(([val, lab]) => /*#__PURE__*/React.createElement("button", {
    key: val,
    onClick: () => patch("sparZielTyp", val),
    style: {
      flex: 1,
      padding: "9px",
      borderRadius: 10,
      ...
      cursor: "pointer",
      fontFamily: "inherit",
      fontWeight: form.sparZielTyp === val ? 700 : 400
    }
  }, lab))), form.sparZielTyp === "betrag" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Monatlicher Sparbetrag (\u20AC)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "25",
    ...
    style: labelStyle
  }, "Prozentsatz des Einkommens (%)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "1",
    max: "80",
    step: "1",
    ...
    placeholder: "z.B. 20",
    style: inputStyle
  })), sparZielAbsolut > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      padding: "10px 14px",
      background: "var(--surface-2)",
  ...
  }, "Sparziel: ", /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "var(--accent)"
    }
  }, fmtEUR(sparZielAbsolut), "/Monat"), form.nettoEinkommen > 0 && ` = ${Math.round(sparZielAbsolut / form.nettoEinkommen * 100)}% deines Einkommens`)), step === 2 && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      marginBottom: 8
    }
  ...
  }, "R\xFCcklage & Sparziele"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: "var(--text-muted)",
      marginBottom: 18,
  ...
  }, "Optional \u2013 du kannst diese jederzeit erg\xE4nzen."), /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Notgroschen-Ziel (\u20AC) \u2013 empfohlen: 3\u20136 Monatsausgaben"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "500",
  ...
  }, "Konkrete Sparziele (optional)"), form.finanzielleZiele.map(z => /*#__PURE__*/React.createElement("div", {
    key: z.id,
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, /*#__PURE__*/React.createElement("strong", null, z.label), z.betrag > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      color: "var(--text-muted)"
    }
  }, " \xB7 ", fmtEUR(z.betrag)), z.bisDatum && /*#__PURE__*/React.createElement("span", {
    style: {
      color: "var(--text-faint)",
      fontSize: 11
    }
  }, " \xB7 bis ", new Date(z.bisDatum).toLocaleDateString("de-DE"))), /*#__PURE__*/React.createElement("button", {
    onClick: () => removeZiel(z.id),
    style: {
      background: "none",
      border: "none",
      cursor: "pointer",
    ...
    type: "text",
    value: neuesZiel.label,
    onChange: e => setNeuesZiel(z => ({
      ...z,
      label: e.target.value
    })),
    placeholder: "Zielname (z.B. Urlaub)",
    style: inputStyle
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
  ...
  }, "\u2190 Zur\xFCck"), step < SCHRITTE.length - 1 ? /*#__PURE__*/React.createElement("button", {
    onClick: () => setStep(s => s + 1),
    style: {
      flex: 2,
      padding: "11px",
...
function BudgetAlertPanel({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, visible.map(alert => /*#__PURE__*/React.createElement("div", {
    key: alert.id,
    role: "button",
    tabIndex: 0,
    onClick: () => {
      if (alert.aktion && onAlertAktion) onAlertAktion(alert);else if (onOpenBot) onOpenBot();
    },
    onKeyDown: e => {
    ...
    style: {
      ...
      borderRadius: 10,
      background: "var(--surface-2)",
      borderLeft: `3px solid ${alert.farbe}`,
      cursor: "pointer"
    }
  ...
  }, alert.text), alert.aktion && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: alert.farbe,
      flexShrink: 0,
...
// ════════════════════════════════════════════════════════════════════════
function BudgetBotSettingsPanel({
  onClose
}) {
  ...
  const RECOMMENDED = [{
    ...
    label: "qwen2.5:7b",
    badge: "🌐 Mehrsprachig",
    desc: "Sehr gutes Deutsch · Finanzthemen stark · ~4 GB"
  }];
  return /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
    ...
    className: "bot-header-title-main"
  }, "KI-Backend"), /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-sub"
  }, "BudgetBot konfigurieren"))), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: "auto",
  ...
  }, "\uD83D\uDCF1 iPhone im Heimnetz:", " ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface-2)",
      padding: "1px 5px",
      borderRadius: 4
    }
  }, "http://192.168.x.x:11434"), " ", "\u2014 Ollama mit", " ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface-2)",
      padding: "1px 5px",
      borderRadius: 4
  ...
  }, "ollama.ai"), " \u2192 Download \u2192 macOS .dmg installieren"), /*#__PURE__*/React.createElement("li", null, "Terminal: ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface)",
      padding: "2px 6px",
      borderRadius: 4
    }
  }, "ollama pull llama3.1:8b")), /*#__PURE__*/React.createElement("li", null, "Ollama l\xE4uft automatisch. URL: ", /*#__PURE__*/React.createElement("code", null, "http://localhost:11434"))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      margin: "10px 0 4px",
      color: "var(--text)"
  ...
  }, /*#__PURE__*/React.createElement("li", null, "Mac-IP: ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface)",
      padding: "2px 6px",
      borderRadius: 4
    }
  }, "ifconfig | grep \"inet 192\"")), /*#__PURE__*/React.createElement("li", null, /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface)",
      padding: "2px 6px",
      borderRadius: 4
    }
  }, "OLLAMA_HOST=0.0.0.0 ollama serve")), /*#__PURE__*/React.createElement("li", null, "URL oben: ", /*#__PURE__*/React.createElement("code", null, "http://192.168.x.x:11434"), " \xB7 gleiches WLAN n\xF6tig")))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "12px 14px",
      borderRadius: 12,
      marginBottom: 20,
  ...
  }, /*#__PURE__*/React.createElement("strong", null, "\u26A1 Offline-Modus immer aktiv"), /*#__PURE__*/React.createElement("br", null), "Wenn Ollama nicht erreichbar ist, antwortet BudgetBot aus der lokalen Analyse-Engine \u2014 20 Budgetthemen, echte Berechnung aus deinen Daten, kein Backend n\xF6tig."), /*#__PURE__*/React.createElement("button", {
    onClick: save,
    style: {
      width: "100%",
      padding: "11px",
...
function BudgetBotModal({
  ...
  React.useEffect(() => {
    pendingMsgRef.current = pendingMessage;
  }, [pendingMessage]);
  ...
  React.useEffect(() => {
    ...
    budgetChatList().then(list => {
      setChats(list);
      if (list.length === 0) _newChat();else {
        _setActiveId(list[0].id);
  ...
  const handleSend = async text => {
    ...
    if (cfg.model && typeof window.ollamaStream === "function") {
      ...
      try {
        antwort = await window.ollamaStream(cfg.url, cfg.model, system, apiMessages, token => {
          setMessages(prev => {
            const copy = [...prev];
  ...
  const handleAlertAktion = alert => {
    if (alert.aktion) handleSend(alert.aktion);
  };
  const handleProfilSaved = newProfil => {
    setProfil(newProfil);
    setWizardOpen(false);
  };
  if (!open) return null;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bot-header-title-main"
  }, "BudgetBot"), /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-sub",
    style: {
      cursor: "pointer"
    },
  ...
  }, profil ? `Profil: ${fmtEUR(profil.nettoEinkommen || 0)}/Monat` : "Kein Profil", " · ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: backend.type === "ollama" ? "var(--accent)" : "var(--text-faint)"
    }
  }, backend.label))), /*#__PURE__*/React.createElement("button", {
    onClick: () => setSettingsOpen(true),
  ...
  }, "\u2715")), profilLoaded && !profil ? /*#__PURE__*/React.createElement("div", {
  ...
  }, "Finanzprofil einrichten"), " \u2013 f\xFCr personalisierte Budgetanalyse."), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: "var(--accent)",
      fontWeight: 600
    }
  }, "Einrichten \u2192")) : profilLoaded && profil ? /*#__PURE__*/React.createElement("div", {
    onClick: () => setWizardOpen(true),
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, "Netto ", fmtEUR(profil.nettoEinkommen || 0), " \xB7 Sparziel ", fmtEUR(profil.sparZielMonatlich || 0), "/Monat", profil.finanzielleZiele?.length > 0 && ` · ${profil.finanzielleZiele.length} Ziel(e)`), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: "var(--text-faint)"
    }
  }, "Profil bearbeiten")) : null, settingsOpen && /*#__PURE__*/React.createElement(BudgetBotSettingsPanel, {
    onClose: () => {
      setSettingsOpen(false);
      setBackend(detectBudgetBackend());
    }
  ...
  }, "Gespr\xE4che"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setSidebarOpen(false),
    style: {
      background: "none",
      border: "none",
  ...
  }, "+ Neues Gespr\xE4ch"), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: "auto"
    }
    ...
    className: "bot-msgs",
    ref: scrollRef
  }, messages.length === 0 && !busy && /*#__PURE__*/React.createElement(BudgetSuggestions, {
    onPick: s => handleSend(s)
  }), messages.map((m, i) => /*#__PURE__*/React.createElement(BudgetMsgBubble, {
    key: i,
    role: m.role,
    content: m.content
  })), busy && /*#__PURE__*/React.createElement(BudgetTypingIndicator, null)), /*#__PURE__*/React.createElement("div", {
    style: {
