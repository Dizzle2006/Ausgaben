...
const _OP_WIRKUNG = {
  ...
  ausbildung: "abzug",
  unterhalt: "abzug",
  aussergewoehnlich: "abzug",
  behinderung: "abzug",
  haushaltsnahe: "credit",
  handwerker: "credit"
};
...
function simulateOptimalPlan(profile, ops, year) {
  ...
  const bbg_kv = K.bbg_kv_monatlich * 12,
    bbg_rv = K.bbg_rv_monatlich * 12;
  const svCfg = (window.TAX_CONFIG_RAW || {}).sv_saetze_an || {};
  const sv = Math.round(Math.min(brutto, bbg_kv) * (svCfg.kv || 0.0735) + Math.min(brutto, bbg_rv) * ((svCfg.rv || 0.093) + (svCfg.alv || 0.013)) + Math.min(brutto, bbg_kv) * (svCfg.pv || 0.018));
  ...
  if (_km > 0 && _aTage > 0) {
    const yrN = Number(yr);
    wkVorhanden += yrN >= 2026 ? Math.round(_aTage * _km * (K.entfernung_km_ab_21 || 0.38)) : Math.round(_aTage * (Math.min(_km, 20) * (K.entfernung_km_bis_20 || 0.30) + Math.max(0, _km - 20) * (K.entfernung_km_ab_21 || 0.38)));
  }
  ...
  function deltaFor(op) {
    ...
    if (wirk === "credit") return {
      delta: Math.round(op.ersparnis || 0),
      wirk,
      abzug: 0
    };
    if (wirk === "fix") return {
  ...
  while (rest.length > 0 && guard++ < 200) {
    ...
    if (!best || best.delta <= 0) {
      ...
      for (const op of rest) plan.push({
        op,
        exakt: 0,
        kumulativ,
        wirkung: _opWirkung(op)
      });
      break;
    }
    ...
    if (best.wirk === "wk") {
      wkSumme += best.abzug;
      zvE = Math.max(0, zvE - (best.wirksam || 0));
    }
    if (best.wirk === "abzug") {
      zvE = Math.max(0, zvE - best.abzug);
    }
  ...
  const creditsFix = plan.filter(x => x.wirkung === "credit" || x.wirkung === "fix").reduce((s, x) => s + x.exakt, 0);
  return {
    plan,
    ...
    zvE_vorher: zvE0,
    zvE_nachher: zvE,
    steuer_vorher: steuer0,
    steuer_nachher: steuer1,
