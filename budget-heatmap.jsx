...
function BudgetHeatmap({ state, monthYM, monthBudget = 0, budgetPeriod }) {
  ...
  const { start: periodStart, lengthDays: daysInMonth } = getPeriodRange(monthYM, period);
  ...
  return (
    <div className="heatmap-card">
      <div className="heatmap-head">
        <div className="heatmap-title">Tagesausgaben</div>
        ...
        {cells.map((c) =>
          c.type === "empty" ? (
            ...
            <span
              ...
              title={c.amount > 0
                ? `${c.day}. — ${fmtEUR(c.amount)}`
                : `${c.day}. — keine Ausgaben`}
              role="gridcell"
              aria-label={`${c.day}. ${c.amount > 0 ? fmtEUR(c.amount) : "keine Ausgaben"}`}
            >
              <span className="heatmap-day-num">{c.day}</span>
            </span>
          )
        )}
      </div>
