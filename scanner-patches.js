...
function detectDuplicate(candidate, existing) {
  ...
  for (const r of existing) {
    ...
    if (cAmt > 0 && cAmt === rAmt && cDate && cDate === rDate) {
      if (_nameMatch(candidate.haendler, r.haendler)) return {
        isDuplicate: true,
        match: r
      };
    }
...
const AutoKat = (() => {
  ...
  function load() {
    try {
      return JSON.parse(localStorage.getItem(AUTOKAT_KEY) || "{}");
    } catch {
      return {};
    }
  }
  function save(map) {
    try {
      localStorage.setItem(AUTOKAT_KEY, JSON.stringify(map));
    } catch {}
  }
