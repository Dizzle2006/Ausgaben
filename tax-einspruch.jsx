...
function EinspruchsGenerator({ receipts, year, tweaks = {}, onClose }) {
  const y = String(year);
  ...
  // Summen aus Belegen
  const wkSumme = (receipts || [])
    .filter((r) => r.steuerkat === "werbungskosten" && (r.datum || "").startsWith(y))
    .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);
  const saSumme = (receipts || [])
    .filter((r) => r.steuerkat === "sonderausgaben" && (r.datum || "").startsWith(y))
    .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);
  const abSumme = (receipts || [])
    .filter((r) => r.steuerkat === "aussergewoehnlich" && (r.datum || "").startsWith(y))
    .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);
  const hdlSumme = (receipts || [])
    .filter((r) => r.steuerkat === "haushaltsnahe" && (r.datum || "").startsWith(y))
    .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);

  const heuteDE = new Date().toLocaleDateString("de-DE",
    { day: "2-digit", month: "2-digit", year: "numeric" });

  const fristDE = bescheiddatum
    ? new Date(new Date(bescheiddatum).getTime() + 30 * 24 * 60 * 60 * 1000)
  ...
  const generiereEinspruch = () => {
    ...
    const belegListe = [];
    if (wkSumme > wkPauschale)
      belegListe.push(`  • Werbungskosten: ${fmtEUR(wkSumme)} (Belege vorhanden)`);
    if (saSumme > 0)
      belegListe.push(`  • Sonderausgaben: ${fmtEUR(saSumme)}`);
    if (abSumme > 0)
      belegListe.push(`  • Außergewöhnliche Belastungen: ${fmtEUR(abSumme)}`);
    if (hdlSumme > 0)
      belegListe.push(`  • Haushaltsnahe Dienstleistungen: ${fmtEUR(hdlSumme)} (20 % Lohnanteil)`);

    const bDatumDE = bescheiddatum
      ? new Date(bescheiddatum).toLocaleDateString("de-DE")
      : "[Bescheiddatum]";

    const text = [
      `Einspruch gemäß § 347 AO`,
      ``,
      `Steuernummer: [Steuernummer eintragen]`,
      `Datum: ${heuteDE}`,
      `Einspruchsfrist: ${fristDE}`,
      ``,
      `Sehr geehrte Damen und Herren,`,
      ``,
      `gegen den Einkommensteuer-Bescheid vom ${bDatumDE} für das Steuerjahr ${y} erhebe ich hiermit form- und fristgerecht`,
      ``,
      `E I N S P R U C H.`,
      ``,
      `Begründung:`,
      ...
      begruendung
        ? begruendung
        : `Der Bescheid berücksichtigt folgende absetzbare Aufwendungen nicht vollständig:`,
      ``,
      ...belegListe,
      ``,
      `Die entsprechenden Belege und Nachweise liegen mir vor und können auf Anforderung eingereicht werden.`,
      ``,
      `Ich beantrage, den Bescheid dahingehend zu ändern, dass die oben genannten Aufwendungen steuermindernd berücksichtigt werden.`,
      ``,
      `Festgesetzte Steuer lt. Bescheid: ${
        festgesetzt ? fmtEUR(Number(festgesetzt)) : "[Betrag lt. Bescheid]"
      }`,
      ``,
    ...
    <div className="tax-modal-backdrop" onClick={onClose}>
      <div className="tax-modal" onClick={(e) => e.stopPropagation()}>
        <div className="tax-modal-header">
          <h2 className="tax-modal-title">Einspruchs-Generator {year}</h2>
        ...
        <div className="tax-modal-body">
          {!brief ? (
            <React.Fragment>
              <div className="einspruch-intro">
                Fülle die Felder aus — der Brief wird automatisch generiert.
                Einspruchsfrist: <strong>1 Monat ab Bescheid-Datum (§ 355 AO)</strong>.
              </div>

              <div className="einspruch-form">
                <label className="bn-field">
                  <span>Datum des Steuerbescheids</span>
                  <input
                    type="date"
                    value={bescheiddatum}
                ...
                <label className="bn-field">
                  <span>Festgesetzte Steuer lt. Bescheid (€)</span>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                ...
                <label className="bn-field" style={{ gridColumn: "1 / -1" }}>
                  <span>Ergänzende Begründung (optional)</span>
                  <textarea
                    rows={4}
                    placeholder='z. B. „Die Werbungskosten wurden nicht berücksichtigt …"'
                    value={begruendung}
              ...
              {(wkSumme > wkPauschale || saSumme > 0 || abSumme > 0 || hdlSumme > 0) && (
                <div className="einspruch-preview">
                  <div className="einspruch-preview-label">Aus deinen Belegen vorausgefüllt</div>
                  {wkSumme > wkPauschale && <div className="einspruch-preview-row">✓ Werbungskosten: {fmtEUR(wkSumme)}</div>}
                  {saSumme > 0 && <div className="einspruch-preview-row">✓ Sonderausgaben: {fmtEUR(saSumme)}</div>}
                  {abSumme > 0 && <div className="einspruch-preview-row">✓ Außergew. Belastungen: {fmtEUR(abSumme)}</div>}
                  {hdlSumme > 0 && <div className="einspruch-preview-row">✓ Haushaltsnahe DL: {fmtEUR(hdlSumme)}</div>}
                </div>
              )}

              <button className="einspruch-generate" onClick={generiereEinspruch}>
                Brief generieren
              </button>
            </React.Fragment>
          ) : (
            <React.Fragment>
              <pre className="einspruch-brief">{brief}</pre>
              <div className="tax-modal-actions">
                <button className="receipt-btn secondary" onClick={() => setBrief(null)}>
                  Bearbeiten
                </button>
                <button
              ...
              <div className="einspruch-footnote">
                Einspruch per Post (Einschreiben) oder per ELSTER einreichen.
                Frist: 1 Monat ab Bescheiddatum (§ 355 AO).
              </div>
            </React.Fragment>
          )}
        </div>
...
function runBescheidCheck(profile, receipts, bescheid = {}, year) {
  ...
  const Kget = K || {};
  const gst = (typeof calcGrenzsteuersatz === "function")
    ? calcGrenzsteuersatz(Math.max(0, (profile.brutto || 0) * 0.72), yr)
    : 0.35;
  ...
  const wkBelege = (receipts || [])
    .filter((r) => r.steuerkat === "werbungskosten" && (r.datum || "").startsWith(String(yr)))
    .reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);
  ...
  if (wkBelege > wkPausch && wkVomFA <= wkPausch) {
    ...
    pruefpunkte.push({
      ...
      befund: wkVomFA > wkPausch
        ? `FA hat ${fmtEUR(wkVomFA)} angesetzt — korrekt (über Pauschale).`
        : `Pauschale ${fmtEUR(wkPausch)} angesetzt — in Ordnung (keine höheren Belege erfasst).`,
      empfehlung: null, paragraph: "§ 9 EStG",
    });
  }

  // 2. §35a-Ermäßigungen
  const hdlBelege = (receipts || [])
    .filter((r) => r.steuerkat === "haushaltsnahe" && (r.datum || "").startsWith(String(yr)));
  const hdlSumme = hdlBelege.reduce((s, r) => s + (Number(r.gesamtbetrag) || 0), 0);
  const ermaessigung35a = r0(hdlSumme * 0.20);
  if (hdlSumme > 0) {
    pruefpunkte.push({
      nr: 2, status: ermaessigung35a > 0 ? "pruefen" : "ok",
  ...
  if (profile.kinder) {
    ...
    const kgMonatlich = yr >= 2026
      ? (raw.kindergeld_monatlich?.ab_2026 ?? K.kindergeld_monatlich)
      : (raw.kindergeld_monatlich?.bis_2025 ?? K.kindergeld_monatlich);
    ...
    pruefpunkte.push({
      ...
      empfehlung: vorteilFreibetrag > kindergeld_jahres
        ? "Anlage Kind muss ausgefüllt sein — Finanzamt macht Günstigerprüfung automatisch."
        : null,
  ...
  if (profile.kirchenmitglied) {
    ...
    pruefpunkte.push({
      ...
      befund: kistVomFA > 0
        ? `FA hat ${fmtEUR(kistVomFA)} KiSt verrechnet.`
        : "Kirchensteuerpflicht laut Profil, aber keine KiSt-Verrechnung im Bescheid erkennbar.",
  ...
  const ORD = { fehler: 0, pruefen: 1, info: 2, ok: 3 };
  return pruefpunkte.sort((a, b) => (ORD[a.status] ?? 9) - (ORD[b.status] ?? 9));
}

function SteuerbescheidPruefer({ profile, receipts, year, onEinspruch }) {
  const [bescheid, setBescheid] = React.useState({});
  const [result, setResult]     = React.useState(null);
  const [showForm, setShowForm] = React.useState(false);

  const STATUS_ICON  = { fehler: "✗", pruefen: "?", ok: "✓", info: "ℹ" };
  const STATUS_COLOR = {
  ...
  return (
    <div style={{
      ...
      {!showForm ? (
        ...
        >
          Bescheid-Daten eingeben &amp; prüfen
        </button>
      ) : (
        <div style={{ display: "grid", gap: "10px", marginBottom: "14px" }}>
          {[
            { key: "festgesetzte_est",         label: "Festgesetzte ESt (€)" },
            { key: "angerechnete_lohnsteuer",  label: "Angerechnete Lohnsteuer (€)" },
            { key: "werbungskosten_angesetzt", label: "WK angesetzt vom FA (€)" },
            { key: "kirchensteuer_verrechnet", label: "KiSt verrechnet (€)" },
          ].map(({ key, label }) => (
            <div key={key} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <label style={{ fontSize: "12px", color: "var(--text-faint)" }}>{label}</label>
              <input
                type="number" min="0"
                value={bescheid[key] || ""}
                onChange={(e) => setBescheid((b) => ({ ...b, [key]: Number(e.target.value) || 0 }))}
                style={{
        ...
        <div>
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px", flexWrap: "wrap" }}>
            {fehlerzahl > 0 && (
              <span style={{
                ...
                background: "oklch(0.95 0.04 25)", color: STATUS_COLOR.fehler, fontWeight: 700,
              }}>
                {fehlerzahl} Fehler
              </span>
            )}
            {pruefzahl > 0 && (
              <span style={{
                padding: "3px 10px", borderRadius: "99px", fontSize: "12px",
                background: "oklch(0.97 0.04 80)", color: STATUS_COLOR.pruefen, fontWeight: 700,
              }}>
                {pruefzahl} zu prüfen
              </span>
            )}
            {fehlerzahl === 0 && pruefzahl === 0 && (
              <span style={{
                padding: "3px 10px", borderRadius: "99px", fontSize: "12px",
                background: "oklch(0.94 0.06 145)", color: STATUS_COLOR.ok, fontWeight: 700,
              }}>
          ...
          {result.map((p) => (
            <div key={p.nr} style={{
              ...
              <span style={{
                fontWeight: 800, color: STATUS_COLOR[p.status],
                fontSize: "15px", minWidth: "18px", marginTop: "1px",
              }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{
                  ...
                  <span>{p.titel}</span>
                  <span style={{ fontSize: "10px", color: "var(--text-faint)", fontWeight: 400, flexShrink: 0 }}>
                    {p.paragraph}
                  </span>
                </div>
                <div style={{ fontSize: "12px", color: "var(--text-secondary, var(--text-muted))", marginTop: "3px", lineHeight: 1.5 }}>
                  {p.befund}
                </div>
                {p.empfehlung && (
                  <div style={{ fontSize: "12px", marginTop: "5px", color: STATUS_COLOR[p.status], fontWeight: 500 }}>
                    → {p.empfehlung}
                  </div>
                )}
              </div>
          ...
          {(fehlerzahl > 0 || pruefzahl > 0) && onEinspruch && (
            <button
              onClick={onEinspruch}
              style={{
...
(function _secureExport() {
  const _defs = { EinspruchsGenerator, runBescheidCheck, SteuerbescheidPruefer };
