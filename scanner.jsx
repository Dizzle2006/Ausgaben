...
async function _ensureTesseract(onProgress) {
  ...
  try {
    ...
    const err = /^(Tesseract\.js konnte nicht|OCR-Initialisierung)/.test(detail)
      ? (e instanceof Error ? e : new Error(detail))
      : new Error(`OCR konnte nicht initialisiert werden:\n${detail}`);
...
function ReceiptCard({ data, categories, image, onChange, onAccept, onDiscard }) {
  ...
  return (
    <div className="receipt-card">
      ...
      <div className="receipt-fields">
        ...
        <div className="receipt-field">
          ...
          <input
            ...
            onChange={(e) => {
              const v = e.target.value;
              update({ mwst_19: v === "" ? 0 : parseFloat(v) });
            }}
      ...
      {data._rawText &&
      <details className="receipt-rawtext">
          <summary>Erkannter Rohtext (OCR)</summary>
          <pre>{data._rawText}</pre>
        </details>
      }
...
function ReceiptScanner({ open, onClose, currentMonth, categories, onAccept, receipts = [] }) {
  ...
  const goBack = () => {
    ...
    else if (step === "crop") { setImage(null); setStep("choose"); }
    else { setStep("choose"); }
  };
    ...
    <div className="scanner-backdrop" onClick={onClose}>
      <div className="scanner-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="scanner-body">
          ...
          {step === "camera" && (
            <CameraView
              onCapture={(data) => { setImage(data); setFileName(null); setStep("crop"); }}
          ...
          {step === "crop" && image && (
            <CropStep
              image={image.dataUrl}
              onConfirm={(cropped) => { setImage(cropped); setStep("preview"); }}
