...
const GITHUB_RAW_URL =
  "https://raw.githubusercontent.com/Dizzle2006/Ausgaben/main/tax-config.json";
...
const INLINE_BOOTSTRAP = {
  meta: {
    ...
    quellen: [
      "BMF Grundfreibetrag 2024-2026",
      "Steueränderungsgesetz 2025",
      "Finanztip, Steuertipps.de (verifiziert Mai 2026)",
      "§32a EStG 2026 (Steuerfortentwicklungsgesetz, verifiziert Mai 2026)",
    ],
  },
  grundfreibetrag: { "2024": 11784, "2025": 12096, "2026": 12348 },
  werbungskostenpauschale: {
    "2024": 1230, "2025": 1230, "2026": 1230,
    hinweis_2026: "Gewerkschaftsbeiträge ab 2026 immer absetzbar, auch unter Pauschale",
  },
  entfernungspauschale: {
    bis_2025: { km_1_bis_20: 0.30, ab_km_21: 0.38 },
    ab_2026:  { ab_km_1: 0.38, hinweis: "Vereinheitlichung ab erstem Kilometer" },
  },
  minijob_grenze_monatlich: { "2024": 538, "2025": 556, "2026": 603 },
  sparerpauschbetrag: { single: 1000, verheiratet: 2000 },
  soli_freigrenze_einzelveranlagung: { "2024": 18130, "2025": 19950, "2026": 20350 },
  kindergeld_monatlich: { bis_2025: 255, ab_2026: 259 },
  kinderfreibetrag_gesamt_2026: 9756,
  kapitalertragsteuer_satz: 0.25,
  soli_satz: 0.055,
  homeoffice: { pauschale_pro_tag: 6, max_tage: 210, max_jahresbetrag: 1260 },
  ...
  kirchensteuer_satz: { BY: 0.08, BW: 0.08, default: 0.09 },
  studenten: {
  ...
  tarifzonen: {
    "2024": {
      zone1_ende: 17005, zone2_ende: 66760, zone3_ende: 277825,
      k1a: 979.18, k1b: 1400,
      k2a: 192.59, k2b: 2397, k2c: 966.53,
      flat42_abzug: 10379, flat45_abzug: 18714,
    },
    "2025": {
      zone1_ende: 17443, zone2_ende: 68480, zone3_ende: 277825,
      k1a: 979.18, k1b: 1400,
      k2a: 192.59, k2b: 2397, k2c: 966.53,
      flat42_abzug: 10602, flat45_abzug: 18936,
    },
    "2026": {
      zone1_ende: 17799, zone2_ende: 69878, zone3_ende: 277825,
      k1a: 979.18, k1b: 1400,
      k2a: 181.19, k2b: 2397, k2c: 1025.38,
      flat42_abzug: 10637.32, flat45_abzug: 18919.93,
    },
  },
  beitragsbemessungsgrenze: {
    "2024": { kv_pv_monatlich: 5175, rv_monatlich: 7550 },
    "2025": { kv_pv_monatlich: 5512, rv_monatlich: 8050 },
    "2026": { kv_pv_monatlich: 5775, rv_monatlich: 8400 },
  },
  ...
  ruerup: {
    hoechstbetrag: 29344,  // § 10 Abs.3 EStG: BBG RV West 2026 × 18,6% × 2
...
function normalizeTaxConfig(raw) {
  ...
  for (const y of years) {
    ...
    out[y] = {
      grundfreibetrag:
        r.grundfreibetrag?.[y] ?? INLINE_BOOTSTRAP.grundfreibetrag[y] ?? 0,
      wk_pauschale:
        r.werbungskostenpauschale?.[y] ?? 1230,
      // nicht in tax-config.json — fester Wert aus § 10c EStG.
      sonderausgaben_pauschale: 36,
      sparerpauschbetrag_single:
        r.sparerpauschbetrag?.single ?? 1000,
      sparerpauschbetrag_verheiratet:
        r.sparerpauschbetrag?.verheiratet ?? 2000,
      homeoffice_pro_tag:
        r.homeoffice?.pauschale_pro_tag ?? 6,
      homeoffice_max_tage:
        r.homeoffice?.max_tage ?? 210,
      entfernung_km_bis_20: ent.km_bis_20,
      entfernung_km_ab_21:  ent.km_ab_21,
      verpflegung_8h:
        r.studenten?.verpflegung_pauschale_8h ?? 14,
      verpflegung_24h:
        r.studenten?.verpflegung_pauschale_24h ?? 28,
      kapest_satz: r.kapitalertragsteuer_satz ?? 0.25,
      soli_satz:   r.soli_satz ?? 0.055,
      kirchensteuer_satz:
        r.kirchensteuer_satz || { BY: 0.08, BW: 0.08, default: 0.09 },
  ...
  Object.defineProperty(out, "__raw", {
    value: frozenRaw,
    enumerable: false,
    writable: false,   // unveränderlich nach Zuweisung
    configurable: false,
  });
  return out;
}
...
async function fetchLatestTaxConfig() {
  ...
  const sources = [
    { name: "GitHub", url: GITHUB_RAW_URL + cacheBust },
    { name: "lokal",  url: LOCAL_CONFIG_URL + cacheBust },
  ];
  ...
  for (const src of sources) {
    try {
      ...
      console.info(
        `%c[Steuern] Konfiguration aktiv: ${src.name} · Stand ${LAST_CONFIG_DATE}`,
        "color:#5b8a3a;font-weight:600"
      );

      window.dispatchEvent(new CustomEvent("tax-config-updated", {
        detail: { source: src.name, date: LAST_CONFIG_DATE, raw },
      }));
      return;
    } catch (e) {
  ...
  console.warn(
    `[Steuern] Keine Remote-Aktualisierung möglich – nutze ${LAST_CONFIG_SOURCE} (Stand ${LAST_CONFIG_DATE})`
  );
}
...
function getK(year) {
  ...
  const knownYears = Object.keys(STEUER_KONSTANTEN)
    .filter((k) => /^\d{4}$/.test(k))
    .sort();
...
// calcBruttoNetto
// ────────────────────────────────────────────────────────────────────────
function calcBruttoNetto({ brutto, steuerklasse = 1, kirchensteuer = false,
                           bundesland = "default", kinder = false, year = 2025 }) {
  const K  = getK(year);
  ...
  const SK_GF_MAP = { 1: true, 2: true, 3: true, 4: true, 5: false, 6: false };
  const sk = {
  ...
  const lohnsteuer  = steuerklasse === 3
    ? Math.round(est_einfach * 2)
    : est_einfach;
  ...
  return {
    ...
    lohnsteuer, soli, kst, kv, rv, alv, pv, sv, abzuege,
  };
}
...
function calcKapESt({ trades = [], year = "2025", verheiratet = false,
                       kirchensteuer = false, bundesland = "default", zve = null }) {
  const K = getK(year);
  const y = String(year);
  ...
  const freibetrag = verheiratet
    ? K.sparerpauschbetrag_verheiratet
    : K.sparerpauschbetrag_single;
...
// ────────────────────────────────────────────────────────────────────────
function calcWK({ receipts = [], year = "2025",
                   arbeitstage = 220, entfernung_km = 0, homeoffice_tage = 0 }) {
  const K = getK(year);
  const y = String(year);

  const belegSumme = (receipts || [])
    .filter((r) => r.steuerkat === "werbungskosten" && (r.datum || "").startsWith(y))
  ...
  const pendler = Math.round(
    pendlertage * (km_zone1 * K.entfernung_km_bis_20 + km_zone2 * K.entfernung_km_ab_21)
  );
...
function buildUserProfile(interviewAnswers, tweaks) {
  ...
  abzugsfaehigePositionen.homeoffice =
    ["arbeitnehmer", "beides", "student_dual", "student_trial"].includes(beschaeftigung);

  // WK-Pauschale
  abzugsfaehigePositionen.werbungskosten_single =
    einkunftsarten.lohnarbeit || hatAusbildungsverhaeltnis;
...
function calcZumutbareEigenbelastung(gesamteinkuenfte, familienstand, kinderAnzahl) {
  const STUFEN = [
    [15340,    0.05, 0.04, 0.02],
    [51130,    0.06, 0.05, 0.03],
    [Infinity, 0.07, 0.06, 0.04],
  ];
...
function TaxConfigStatusBanner() {
  const { source: configSource, date: configDate } = useTaxConfig();
  ...
  return (
    <div style={{
      ...
      <span>{isOffline ? "⚠" : "ℹ"}</span>
      <span style={{ flex: 1 }}>
        {isOffline
          ? `Steuer-Werte: Offline-Fallback (Stand ${configDate}). Internetverbindung herstellen für aktuelle Sätze.`
          : stale
          ? `Steuer-Konfiguration ist älter als 30 Tage (Stand: ${configDate}). Lade die App neu für aktuelle Werte.`
          : `Steuer-Werte geladen (Stand: ${configDate})`
        }
      </span>
      {stale && (
        <button
          onClick={() => { localStorage.removeItem("tax_constants_raw"); window.location.reload(); }}
      ...
      <button
        ...
        style={{ background: "none", border: "none", cursor: "pointer",
          color: "var(--text-faint)", fontSize: "14px", lineHeight: 1, fontFamily: "inherit" }}
...
function BruttoNettoRechner({
  ...
  const r = React.useMemo(
    () => calcBruttoNetto({
      brutto, steuerklasse: sk, kirchensteuer: kst,
      bundesland: defaultBundesland, kinder, year,
    }),
  ...
  const rows = [
    ...
    ["Kirchensteuer",       r.kst],
    ["Krankenversicherung", r.kv],
    ["Rentenversicherung",  r.rv],
    ["Arbeitslosenvers.",   r.alv],
    ["Pflegeversicherung",  r.pv],
  ];

  return (
    <div className="bn-rechner">
      {/* Eingaben */}
      <div className="bn-inputs">
        <label className="bn-field">
          <span>Jahresbrutto</span>
          <input
            type="number"
            step="500"
            min="0"
        ...
        <label className="bn-field">
          ...
          <select value={sk} onChange={(e) => setSk(Number(e.target.value))}>
            {[1,2,3,4,5,6].map((n) => <option key={n} value={n}>Klasse {n}</option>)}
          </select>
        </label>
        <label className="bn-field">
          <span>Jahr</span>
          <select value={year} onChange={(e) => setYear(Number(e.target.value))}>
            {Object.keys(STEUER_KONSTANTEN)
              .filter((y) => /^\d{4}$/.test(y))
              .sort()
              .map((y) =>
      ...
      <div className="bn-checks">
        <label className="bn-check">
          <input type="checkbox" checked={kst} onChange={(e) => setKst(e.target.checked)} />
          <span>Kirchensteuer</span>
        </label>
        <label className="bn-check">
          <input type="checkbox" checked={kinder} onChange={(e) => setKinder(e.target.checked)} />
          <span>Mit Kindern (geringerer PV-Satz)</span>
        </label>
      </div>
      ...
      <div className="bn-rows">
        {rows.map(([label, val]) => (
          <div key={label} className={`bn-row ${val === 0 ? "muted" : ""}`}>
            <span className="bn-row-label">{label}</span>
            <span className="bn-row-value">
              − {fmtEUR(val)}
              <span className="bn-row-month"> / {monat(val)}</span>
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="bn-result">
        <div>
          <div className="bn-result-label">Netto / Jahr</div>
          <div className="bn-result-month">{monat(r.netto)} / Monat</div>
        </div>
        <div className="bn-result-value">{fmtEUR(r.netto)}</div>
      </div>
    </div>
  );
}

function KapEStCard({ trades = [], year, tweaks = {} }) {
  const r = calcKapESt({
    trades, year,
    ...
    <div className="section">
      <div className="section-header">
        <div className="section-title">
          ...
          <h2>Kapitalertragsteuer {year}</h2>
        </div>
        <div className="section-total" style={{ color: r.gesamt > 0 ? "var(--danger)" : "var(--text-faint)" }}>
          {r.gesamt > 0 ? `− ${fmtEUR(r.gesamt)}` : "—"}
        </div>
      </div>
      <div className="tax-card-body">
        <div className="tax-row">
          <span>Realisierte Gewinne</span>
          <span className="num pos">+ {fmtEUR(r.gewinn)}</span>
        </div>
        {r.verlust > 0 && (
          <div className="tax-row">
            <span>Realisierte Verluste</span>
            <span className="num neg">− {fmtEUR(r.verlust)}</span>
          </div>
        )}
        <div className="tax-row">
          <span>Sparerpauschbetrag</span>
          <span className="num muted">− {fmtEUR(r.freibetrag)}</span>
        </div>
        <div className="tax-row strong">
          <span>Steuerpflichtig</span>
          <span className="num">{fmtEUR(r.steuerpflichtig)}</span>
        </div>
        {r.kapest > 0 && (
          <React.Fragment>
            <div className="tax-row">
              <span>KapESt 25%</span>
              <span className="num neg">− {fmtEUR(r.kapest)}</span>
            </div>
            {r.soli > 0 && (
              <div className="tax-row">
                <span>Solidaritätszuschlag</span>
                <span className="num neg">− {fmtEUR(r.soli)}</span>
              </div>
            )}
            {r.kst > 0 && (
              <div className="tax-row">
                <span>Kirchensteuer</span>
                <span className="num neg">− {fmtEUR(r.kst)}</span>
              </div>
            )}
          </React.Fragment>
        )}
        {r.guenstigerHinweis && (
          <div className="tax-hint warning" style={{ marginTop: 10 }}>
            Günstigerprüfung prüfen: Bei niedrigem Grenzsteuersatz (&lt; 25%) kann
            Anlage KAP mit Günstigerprüfung günstiger sein.
          </div>
        )}
      </div>
    </div>
...
function classifyArbeitsort(name) {
  ...
  if (!n) return { typ: "unbekannt", label: "", deductible: null };

  // Privat / nicht absetzbar
  const privat = [
    /sport(halle|verein|platz|studio|club|park)?/, /fitnessstudio/, /\bgym\b/, /\bgymnasium\b(?!.*schule)/,
    /schwimmbad/, /freibad/, /fußball(platz|verein)?/, /tennisplatz/, /kino/, /theater/,
    /restaurant/, /café|cafe/, /\bbar\b/, /disco/, /club(?!.*haus)/, /shopping/,
    /zuhause/, /privat/, /\bhome\b/, /wohnung/, /eltern/,
  ];
  if (privat.some((r) => r.test(n))) {
    return { typ: "privat", label: "Privat — nicht absetzbar", deductible: false, color: "#d64040" };
  }

  // Ausbildung / duales Studium → auch absetzbar (Auswärtstätigkeit § 9 EStG)
  const ausbildung = [
    /berufsschule/, /\bbbs\b/, /\bbsz\b/, /berufliche schule/, /berufskolleg/,
    /universität/, /\buni\b/, /hochschule/, /\bfh\b/, /fachhochschule/,
    /\bdhbw\b/, /\btu\b(?! berlin)/, /\brwth\b/, /\bkit\b/, /duale hochschule/,
    /akademie/, /campus/, /bildungszentrum/, /schulungszentrum/, /lernzentrum/,
    /ausbildungsbetrieth?/, /\bazubi\b/, /auszubildend/,
  ];
  ...
  const beruflich = [
    /büro/, /office/, /\bbetrieb\b/, /firma/, /unternehmen/, /\bgmbh\b/, /\bag\b/,
    /\bkg\b/, /\bohnebh\b/, /arbeit(s|geber|stätte|ort|platz)?/, /\bdienst\b/,
    /filiale/, /niederlassung/, /\bwerk\b/, /fabrik/, /lager/, /werkstatt/,
    /\bkunde\b/, /kundentermin/, /außendienst/, /dienstreise/, /hauptstandort/,
    /arbeitgeber/, /beschäftigungs/, /\babteilung\b/, /hauptsitz/, /\bstandort\b/,
  ];
  ...
  return { typ: "unbekannt", label: "Bitte prüfen", deductible: null, color: "var(--text-muted)" };
}

function PauschaleCard({ receipts = [], year, tweaks = {} }) {
  const _nextId = React.useRef(1);
  const mkId = () => _nextId.current++;

  const [standorte, setStandorte] = React.useState(() => [
    { id: mkId(), label: "Standort 1", km: tweaks.km_einweg || 0, arbeitstage: tweaks.arbeitstage || 220 }
  ]);
  const [hoTage, setHoTage] = React.useState(tweaks.homeoffice_tage || 0);

  const addStandort = () => setStandorte((prev) => [
    ...prev, { id: mkId(), label: `Standort ${prev.length + 1}`, km: 0, arbeitstage: 0 }
  ]);
  const removeStandort = (id) => setStandorte((prev) => prev.filter((s) => s.id !== id));
  const updateStandort = (id, patch) =>
    setStandorte((prev) => prev.map((s) => s.id === id ? { ...s, ...patch } : s));
  ...
  const wk = React.useMemo(() => {
    ...
    const pendlerpauschale = abzugsfaehig.reduce((sum, s) => {
      ...
      if (Number(year) >= 2026) {
        ...
        zone1 = Math.min(km, 20); zone2 = Math.max(0, km - 20);
      }
    ...
    <div className="section">
      <div className="section-header">
        <div className="section-title">
          ...
          <h2>Werbungskosten & Pauschalen {year}</h2>
        </div>
        <div className="section-total" style={{ color: wk.lohntSichEinzeln ? "var(--accent)" : "var(--text-faint)" }}>
          {fmtEUR(Math.max(wk.wk_gesamt, wk.pauschale))}
        </div>
      </div>
      <div className="tax-card-body">
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em",
            ...
            Arbeitsweg / Standorte
          </div>
          {standorte.map((s, idx) => {
            const cls = classifyArbeitsort(s.label);
            const isPrivat = cls.deductible === false;
            return (
              <div key={s.id} style={{ marginBottom: 10, background: "var(--surface-2)", borderRadius: 9, padding: "8px 10px",
                border: isPrivat ? "1.5px solid #d64040" : "1px solid var(--border)" }}>
                {/* Zeile 1: Bezeichnung + Löschen-Button */}
                <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {idx === 0 && (
                      <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 2 }}>Bezeichnung (z.B. Büro, Berufsschule, Uni)</div>
                    )}
                    <input
                      type="text"
                      className="bn-field"
                      ...
                      value={s.label}
                      onChange={(e) => updateStandort(s.id, { label: e.target.value })}
                  ...
                  {standorte.length > 1 && (
                    <button
                      ...
                      style={{
                        ...
                        color: "var(--danger)", fontSize: 14, lineHeight: 1, height: 34
                      }}>
                      ×
                    </button>
                  )}
                ...
                {s.label.trim().length > 0 && (
                  <div style={{
                    ...
                    color: cls.color || "var(--text-muted)", marginBottom: 6,
                    ...
                    <span>{isPrivat ? "✗" : cls.deductible ? "✓" : "?"}</span>
                    <span>{cls.label}</span>
                  </div>
                )}
                {/* Hinweis-Text für duales Studium / Auswärtstätigkeit */}
                {cls.hint && s.label.trim().length > 0 && (
                  <div style={{ fontSize: 10.5, color: "var(--text-faint)", marginBottom: 6, lineHeight: 1.4 }}>
                    {cls.hint}
                  </div>
                )}
                {/* Zeile 2: km + Tage */}
                <div style={{ display: "flex", gap: 8 }}>
                  <label style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 2 }}>km (einfach)</div>
                    <input
                      ...
                      value={s.km}
                      disabled={isPrivat}
                      onChange={(e) => updateStandort(s.id, { km: Number(e.target.value) })}
                    />
                  </label>
                  <label style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 2 }}>Tage / Jahr</div>
                    <input
                      type="number" min="0" max="260" step="1"
                      className="bn-field"
                      style={{ width: "100%", boxSizing: "border-box",
                        opacity: isPrivat ? 0.4 : 1 }}
                      value={s.arbeitstage}
                      disabled={isPrivat}
                      onChange={(e) => updateStandort(s.id, { arbeitstage: Number(e.target.value) })}
                    />
                  </label>
                </div>
                {isPrivat && (
                  <div style={{ fontSize: 11, color: "#d64040", marginTop: 5, fontWeight: 600 }}>
                    Dieser Ort gilt als privat — wird nicht in der Pendlerpauschale berücksichtigt.
                  </div>
                )}
              </div>
            );
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 7, cursor: "pointer", color: "var(--text-muted)", fontWeight: 600
            }}>
            + Standort hinzufügen
          </button>
        </div>

        <label className="bn-field" style={{ marginBottom: 12 }}>
          <span>Homeoffice-Tage</span>
          <input type="number" min="0" max="210" step="1"
            value={hoTage} onChange={(e) => setHoTage(Number(e.target.value))} />
        </label>

        <div className="tax-row">
          <span>Belege (Werbungskosten)</span>
          <span className="num">{fmtEUR(wk.belegSumme)}</span>
        </div>
        {wk.pendlerpauschale > 0 && (
          <div className="tax-row">
            <span>Pendlerpauschale{standorte.length > 1 ? " (alle Standorte)" : ""}</span>
            <span className="num pos">+ {fmtEUR(wk.pendlerpauschale)}</span>
          </div>
        )}
        {wk.homeoffice_pauschale > 0 && (
          <div className="tax-row">
            <span>Homeoffice-Pauschale</span>
            <span className="num pos">+ {fmtEUR(wk.homeoffice_pauschale)}</span>
          </div>
        )}
        <div className="tax-row strong">
          <span>WK gesamt</span>
          <span className="num">{fmtEUR(wk.wk_gesamt)}</span>
        </div>
        <div className="tax-row">
          <span>WK-Pauschale (automatisch)</span>
          <span className="num muted">{fmtEUR(wk.pauschale)}</span>
        </div>

        <div className={`tax-hint ${wk.lohntSichEinzeln ? "success" : "warning"}`} style={{ marginTop: 10 }}>
          {wk.lohntSichEinzeln
            ? `✓ Einzeln abrechnen lohnt sich: ${fmtEUR(wk.vorteil)} mehr als die Pauschale.`
            : `Pauschale reicht — deine WK (${fmtEUR(wk.wk_gesamt)}) liegen unter ${fmtEUR(wk.pauschale)}.`}
        </div>
      </div>
    </div>
  );
...
(function _secureExport() {
  ...
  const valueDefs = {
    ...
    getK, calcESt, calcBruttoNetto, calcKapESt, calcWK,
    BruttoNettoRechner, KapEStCard, PauschaleCard,
  ...
  for (const [k, v] of Object.entries(valueDefs)) {
    ...
    } catch { window[k] = v; /* Fallback falls Property bereits existiert */ }
  }

  // Getter-Exports: Live-Werte die sich nach fetchLatestTaxConfig() ändern
  const getterDefs = {
    STEUER_KONSTANTEN: { get() { return STEUER_KONSTANTEN; } },
    TAX_CONFIG_RAW:    { get() { return STEUER_KONSTANTEN.__raw; } },
    TAX_CONFIG_SOURCE: { get() { return LAST_CONFIG_SOURCE; } },
    TAX_CONFIG_DATE:   { get() { return LAST_CONFIG_DATE; } },
  };
  for (const [k, desc] of Object.entries(getterDefs)) {
    try {
      Object.defineProperty(window, k, { ...desc, configurable: false, enumerable: true });
