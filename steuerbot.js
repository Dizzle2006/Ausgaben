...
function buildUserContext({
  ...
  if (brutto > 0) {
    if (typeof _calcPreciseGST === "function") {
      grenzsteuersatz = Math.round(_calcPreciseGST({
        brutto,
        _ia: ia
      }, year) * 100);
      ...
      const sv = Math.round(Math.min(brutto, bbgKv) * Kfb.sv_kv + Math.min(brutto, bbgRv) * (Kfb.sv_rv + Kfb.sv_alv) + Math.min(brutto, bbgKv) * Kfb.sv_pv);
  ...
  // ─────────────────────────────────────────────────────────────
  const lines = [`<user_kontext>`, `Steuerjahr: ${year}`, ``, `=== PERSÖNLICHES STEUER-PROFIL ===`, `Steuerklasse: ${sk}`, `Familienstand: ${fam}`, `Bundesland: ${blLabel}${bundesland ? ` (${bundesland})` : ""}`, `Kirchensteuerpflichtig: ${kst ? `ja (${kistSatz} %)` : "nein"}`, `Beschäftigung: ${berufstyp}`, `Kinder: ${kinder > 0 ? kinder : "keine"}`, ``];
  if (brutto > 0) {
  ...
  if (stCtxRaw.length > 0) {
    ...
    ctxLocs = [];
    if (pendler_km > 0) ctxLocs.push({
      name: "Hauptstandort",
      km: pendler_km,
      tage: p1tage
    });
    if (pendler2_km > 0) ctxLocs.push({
...
const _SEMANTIC = {
  homeoffice: {
    w: 3,
    kw: ["zuhause", "daheim", "home", "remote", "wohnung", "wohnzimmer", "schreibtisch"]
  },
  pendler: {
    w: 2,
    kw: ["fahren", "fahrweg", "strecke", "bus", "bahn", "auto", "fahrrad", "rad", "km", "kilometer", "büro", "weg"]
  },
  bruttoNetto: {
    w: 2,
    kw: ["gehalt", "lohn", "verdienen", "verdienst", "ausgezahlt", "netto", "brutto", "abzug", "abzüge", "monat"]
  },
  kapital: {
    w: 3,
    kw: ["aktien", "etf", "depot", "fonds", "dividende", "zinsen", "gewinn", "verlust", "broker", "börse", "wertpapier", "anlage"]
  },
  kinder: {
    w: 3,
    kw: ["kind", "kinder", "sohn", "tochter", "kita", "krippe", "schule", "tagesmutter", "hort", "betreuung", "eltern"]
  },
  haushaltsnahe: {
    w: 3,
    kw: ["putzen", "reinigen", "putzhilfe", "garten", "renovier", "handwerker", "reparatur", "sanitär", "elektriker", "maler", "haushalt", "wohnung"]
  },
  aussergewoehnlich: {
    w: 3,
    kw: ["arzt", "zahnarzt", "krankenhaus", "medikamente", "kur", "rollstuhl", "hilfsmittel", "pflege", "behinderung", "krankheit", "kosten"]
  },
  arbeitsmittel: {
    w: 3,
    kw: ["laptop", "computer", "pc", "notebook", "monitor", "drucker", "stuhl", "schreibtisch", "headset", "tablet", "handy", "buch", "literatur"]
  },
  sonderausgaben: {
    w: 3,
    kw: ["versicherung", "krankenversicherung", "kv", "spende", "kirchensteuer", "schule", "ausbildung", "fortbildung", "weiterbildung"]
  },
  riester: {
    w: 4,
    kw: ["riester", "zulage", "grundzulage", "kinderzulage", "altersvorsorge", "förderung", "anlage av"]
  },
  bav: {
    w: 4,
    kw: ["betriebliche", "betriebsrente", "entgeltumwandlung", "direktversicherung", "arbeitgeber", "rente", "bav"]
  },
  fristen: {
    w: 3,
    kw: ["frist", "termin", "abgabe", "wann", "deadline", "31.", "juli", "datum", "einreichen", "verpflichtet"]
  },
  optimieren: {
    w: 2,
    kw: ["sparen", "absetzen", "zurück", "rückerstattung", "tipp", "besser", "mehr", "lohnt", "sinnvoll", "möglichkeit", "optimieren"]
  },
  steuerklasse: {
    w: 3,
    kw: ["klasse", "steuerklasse", "verheiratet", "partner", "ehe", "trennung", "alleinerziehend", "faktor"]
  },
  doppelteHH: {
    w: 3,
    kw: ["andere stadt", "zweitwohnung", "zweite wohnung", "wochenende", "pendeln weit", "weit entfernt", "arbeitsort", "unterkunft", "miete am arbeits"]
  },
  studenten: {
    w: 3,
    kw: ["studium", "uni", "hochschule", "student", "ausbildung", "azubi", "bachelor", "master", "promotion", "bafög", "semester"]
  },
  unterhalt: {
    w: 4,
    kw: ["unterhalt", "ex-frau", "ex-mann", "ex-partner", "geschieden", "getrennt", "scheidung", "kind zahlen"]
  },
  verlust: {
    w: 3,
    kw: ["verlust", "minus", "verlustbescheinigung", "verlustverrechnung", "negativ"]
  },
  minijob: {
    w: 3,
    kw: ["nebenjob", "minijob", "nebenbei", "nebenher", "geringfügig", "midi", "zusatzjob", "zweiter job"]
  },
  grundfreibetrag: {
    w: 3,
    kw: ["freibetrag", "freigrenze", "steuerfrei", "grenze", "erst ab", "keine steuern", "grundbetrag"]
  },
  kirchensteuer: {
    w: 4,
    kw: ["kirche", "kirchensteuer", "evangelisch", "katholisch", "konfession", "austritt", "kirchenmitglied"]
  },
  soli: {
    w: 4,
    kw: ["soli", "solidaritätszuschlag", "solidaritäts"]
  }
};

// Normalisiert: Kleinschreibung, Umlaute standardisieren, häufige Tippfehler korrigieren
function _normalize(msg) {
  return msg.toLowerCase().replace(/ä/g, "ae").replace(/ö/g, "oe").replace(/ü/g, "ue").replace(/ß/g, "ss").replace(/absetzten/g, "absetzen") // häufiger Tippfehler
  .replace(/steurern?/g, "steuern").replace(/gehallt/g, "gehalt").replace(/nto\b/g, "netto").replace(/brutoo?/g, "brutto");
}
...
function _extractSlots(rawMsg) {
  ...
  while ((m = numRe.exec(msg)) !== null) {
    ...
    if (/^\s*(arbeits|b[üu]ro|homeoffice|ho)?-?\s*tage?n?\b/.test(after)) {
      ...
      if (/tage?n?\s*(die|pro|je|in der)\s*woche/.test(afterFull)) {
        if (slots.tage_pro_woche == null) slots.tage_pro_woche = v;
      } else if (/pendel|pendle|fahr|büro|arbeitsweg/.test(nahVor)) {
        if (slots.tage == null) slots.tage = v;
      } else if (/home.?o?f+ice|zu\s*hause|zuhause|daheim|remote/.test(w)) {
        if (slots.ho_tage == null) slots.ho_tage = v;
      } else if (slots.tage == null) {
        slots.tage = v;
      }
      continue;
    }
    ...
    if (/^\s*%/.test(after) || /\bgdb\s*(von\s*)?$/.test(before)) {
      if (/gdb|behinder/.test(w)) slots.gdb = v;else slots.prozent = v;
      continue;
    }
    // Kinderzahl
    if (/^\s*kind(er)?\b/.test(after) && v <= 10) {
      slots.kinder = Math.round(v);
      continue;
    }
    // Geldbeträge
    const istGeld = /^\s*(€|euro|eur\b)/.test(after) || m[2] || /(brutto|gehalt|verdien|einkommen|lohn|jahresgehalt|verdienst)\s*(von|betr[äa]gt|ist|liegt bei|:)?\s*$/.test(before) || /^\s*(€|euro)?\s*(brutto|gehalt|im jahr|j[äa]hrlich|pro jahr|im monat|monatlich|verdien)/.test(afterFull);
    if (istGeld) {
      let geld = v;
      ...
      if (geld >= 3000) {
        ...
        slots.bruttoList.push(geld);
        if (slots.brutto == null) slots.brutto = geld;
      } else {
        slots.geldList = slots.geldList || [];
        slots.geldList.push(geld);
      }
      continue;
    }
  }
...
function _fuzzyIntents(msg) {
  ...
  return Object.entries(scores).filter(([, s]) => s >= 3).sort((a, b) => b[1] - a[1]).map(([k]) => k);
}
...
const _TRAIN = {
  homeoffice: ["ich arbeite von zuhause", "tageweise im homeoffice taetig", "remote arbeiten steuer", "arbeite daheim am schreibtisch", "wieviel gibt es pro heimarbeitstag", "von zu hause aus angestellt", "mobiles arbeiten absetzen", "tagespauschale heimarbeit", "ich bin oft im home office", "arbeitstage in der wohnung"],
  pendler: ["ich fahre jeden tag zur arbeit", "weg ins buero absetzen", "kilometer zur firma", "taeglicher arbeitsweg mit dem auto", "mit der bahn zur arbeitsstelle", "fahrtkosten geltend machen", "entfernung wohnung arbeitsstaette", "strecke zum arbeitsplatz", "benzinkosten arbeitsweg", "ich pendle in eine andere stadt"],
  werbungskosten: ["berufliche ausgaben absetzen", "kosten fuer den job geltend machen", "was zaehlt zu den werbungskosten", "ausgaben fuer die arbeit zurueckholen", "beruflich veranlasste kosten", "ueber die pauschale kommen", "einzelnachweis statt pauschbetrag", "arbeitgeber erstattet kosten nicht"],
  bruttoNetto: ["was bleibt von meinem gehalt uebrig", "wieviel wird mir ausgezahlt", "abzuege vom lohn berechnen", "monatliches netto ausrechnen", "vom brutto zum netto", "was kommt aufs konto", "gehalt nach steuern und abgaben", "lohnabrechnung verstehen", "wie hoch sind meine abgaben"],
  steuerklasse: ["welche lohnsteuerklasse ist richtig", "nach der hochzeit klasse wechseln", "drei fuenf oder vier vier", "faktorverfahren fuer ehepaare", "steuerklassen kombination ehepartner", "alleinerziehend welche klasse", "klassenwechsel beantragen", "lohnsteuerabzug zu hoch klasse pruefen"],
  grundfreibetrag: ["ab wann muss ich steuern zahlen", "bis zu welchem betrag steuerfrei", "steuerfreies existenzminimum", "wieviel darf ich verdienen ohne steuer", "untergrenze fuer einkommensteuer", "wann beginnt die steuerpflicht"],
  sonderausgaben: ["versicherungen von der steuer absetzen", "krankenkassenbeitraege geltend machen", "spenden in der steuererklaerung", "vorsorgeaufwendungen eintragen", "kirchensteuer als sonderausgabe", "weiterbildungskosten privat absetzen"],
  haushaltsnahe: ["putzhilfe von der steuer absetzen", "handwerkerrechnung geltend machen", "reinigungskraft im haushalt steuer", "gartenarbeit absetzen", "renovierung in der wohnung steuerlich", "hausmeisterkosten nebenkostenabrechnung", "fensterputzer steuerermaessigung", "pflegekraft im haushalt"],
  kapital: ["gewinne aus aktien versteuern", "etf verkauft was nun", "dividenden in der steuer", "zinsen vom tagesgeld melden", "depot gewinne steuer", "abgeltungsteuer auf wertpapiere", "freistellungsauftrag bei der bank", "kursgewinne realisiert steuerpflichtig", "thesaurierender fonds besteuerung"],
  guenstiger: ["kapitalertraege mit persoenlichem steuersatz", "guenstigerpruefung beantragen", "weniger als 25 prozent auf kapital zahlen", "anlage kap antrag niedriger steuersatz"],
  soli: ["muss ich noch solidaritaetszuschlag zahlen", "soli abgeschafft fuer wen", "freigrenze beim soli", "zuschlag auf die lohnsteuer"],
  kirchensteuer: ["wieviel kirchensteuer zahle ich", "aus der kirche austreten steuer sparen", "kirchenmitglied abgaben", "konfession und steuerabzug"],
  fristen: ["bis wann muss die steuererklaerung abgegeben werden", "abgabetermin dieses jahr", "deadline fuer die erklaerung", "frist verpasst was tun", "verlaengerung beim finanzamt", "bin ich zur abgabe verpflichtet"],
  minijob: ["nebenjob auf geringfuegiger basis", "verdienstgrenze beim minijob", "zweiter job neben der hauptstelle", "midijob uebergangsbereich", "nebenbei etwas dazuverdienen steuer"],
  riester: ["riester vertrag lohnt sich das", "zulagen fuer altersvorsorge", "grundzulage kinderzulage beantragen", "riester in der steuererklaerung"],
  ruerup: ["basisrente fuer selbststaendige", "ruerup beitraege absetzen", "altersvorsorge ohne gesetzliche rente", "freiberufler rente steuerlich"],
  bav: ["betriebliche altersvorsorge ueber den arbeitgeber", "entgeltumwandlung sinnvoll", "direktversicherung vom chef", "brutto in die betriebsrente einzahlen"],
  aussergewoehnlich: ["hohe arztkosten absetzen", "zahnarztrechnung steuererklaerung", "krankheitskosten geltend machen", "brille und medikamente absetzen", "pflege der eltern steuerlich", "behindertenpauschbetrag beantragen", "zumutbare belastung ueberschritten"],
  studenten: ["als student steuern zurueckholen", "studienkosten geltend machen", "erststudium oder zweitstudium absetzen", "werkstudent lohnsteuer", "semesterbeitraege und fachbuecher", "azubi steuererklaerung lohnt sich"],
  verlust: ["verluste mit gewinnen verrechnen", "aktienverlust steuerlich nutzen", "verlustbescheinigung bei der bank", "minus aus dem depot vortragen", "verlustvortrag aus dem studium"],
  arbeitsmittel: ["laptop fuer die arbeit gekauft", "buerostuhl absetzen", "monitor beruflich angeschafft", "handy dienstlich genutzt", "fachbuecher fuer den beruf", "technik fuer den job abschreiben"],
  doppelteHH: ["zweitwohnung am arbeitsort", "wochenendpendler zwei wohnungen", "miete am beschaeftigungsort absetzen", "beruflich in anderer stadt wohnen"],
  unterhalt: ["unterhalt an die ex frau absetzen", "zahlungen nach der scheidung steuerlich", "realsplitting anlage u", "kindesunterhalt in der steuer"],
  kinder: ["kindergeld oder kinderfreibetrag", "kitagebuehren absetzen", "betreuungskosten fuer den sohn", "alleinerziehend mit tochter entlastung", "was bekomme ich fuer meine kinder"],
  rueckwirkend: ["steuererklaerung fuer alte jahre nachreichen", "noch nie eine erklaerung abgegeben", "rueckwirkend geld vom finanzamt holen", "vier jahre frist freiwillige abgabe"],
  elster: ["wie reiche ich die erklaerung online ein", "elster konto registrieren", "formulare digital ausfuellen", "steuererklaerung uebers internet abgeben", "welche anlage brauche ich"],
  optimieren: ["wie hole ich am meisten raus", "alle sparmoeglichkeiten zeigen", "steuerlast senken tipps", "mehr erstattung vom finanzamt", "was kann ich alles absetzen", "potenzial in meiner steuer finden", "geld zurueck maximieren"]
};
...
const _INTENT_LABELS = {
  ...
  pendler: "Pendlerpauschale",
  werbungskosten: "Werbungskosten",
  bruttoNetto: "Brutto-Netto-Rechnung",
  steuerklasse: "Steuerklassen",
  grundfreibetrag: "Grundfreibetrag/Freibeträge",
  sonderausgaben: "Sonderausgaben",
  haushaltsnahe: "Haushaltsnahe/Handwerker (§ 35a)",
  kapital: "Kapitalerträge/ETF",
  guenstiger: "Günstigerprüfung",
  soli: "Solidaritätszuschlag",
  kirchensteuer: "Kirchensteuer",
  fristen: "Abgabefristen",
  minijob: "Minijob/Midijob",
  riester: "Riester-Rente",
  ruerup: "Rürup/Basisrente",
  bav: "Betriebliche Altersvorsorge",
  aussergewoehnlich: "Außergewöhnliche Belastungen",
  studenten: "Studenten/Azubis",
  verlust: "Verluste/Verlustvortrag",
  arbeitsmittel: "Arbeitsmittel",
  doppelteHH: "Doppelte Haushaltsführung",
  unterhalt: "Unterhalt",
  kinder: "Kinder/Kindergeld",
  rueckwirkend: "Rückwirkende Erklärungen",
  elster: "ELSTER/Formulare",
  optimieren: "Optimierungs-Analyse"
};
...
const _REQUIRED_SLOTS = {
  pendler: {
    check: p => p.pendlKm > 0,
    frage: "Wie viele Kilometer ist die **einfache Strecke** zur Arbeit? (z.B. „18 km“ — gern auch mit Arbeitstagen: „18 km an 200 Tagen“)",
    slot: "km"
  },
  homeoffice: {
    check: p => p.hoTage > 0,
    frage: "An wie vielen **Tagen im Jahr** arbeitest du im Homeoffice? (z.B. „120 Tage“ oder „3 Tage die Woche“)",
    slot: "ho_tage"
  },
  bruttoNetto: {
    check: p => p.brutto > 0,
    frage: "Wie hoch ist dein **Jahresbrutto**? (z.B. „48.000 €“ oder „4.000 € im Monat“)",
    slot: "brutto"
  }
};
...
function _applySlots(p, slots, msg, yr, K) {
  ...
  if (slots.km != null && slots.km > 0 && slots.km <= 400) {
    q.pendlKm = slots.km;
    info.push(slots.km + " km");
  }
  if (slots.tage != null && slots.tage > 0 && slots.tage <= 366) {
    q.atage = Math.round(slots.tage);
    info.push(Math.round(slots.tage) + " Arbeitstage");
  }
  ...
  if (slots.sk != null) {
    q.sk = slots.sk;
    info.push("SK " + slots.sk);
  }
  if (slots.kinder != null) {
    q.kinder = slots.kinder > 0;
    if (slots.kinder > 0) info.push(slots.kinder + (slots.kinder === 1 ? " Kind" : " Kinder"));
  }
...
function _h_vergleichBrutto(list, p, yr, K) {
  ...
  const a = Math.round(list[0]),
    b = Math.round(list[1]);
  const ca = calcBruttoNetto({
    brutto: a,
    steuerklasse: p.sk,
    kirchensteuer: p.kstMit,
    bundesland: p.bl,
    kinder: p.kinder,
    year: yr
  });
  const cb = calcBruttoNetto({
    brutto: b,
    steuerklasse: p.sk,
    kirchensteuer: p.kstMit,
    bundesland: p.bl,
...
function _h_vergleichKm(list, p, yr, K) {
  ...
  const pa = _pendler(a, tage, yr, K),
    pb = _pendler(b, tage, yr, K);
...
function _h_vergleichSK(b1, b2, p, yr, K) {
  ...
  const n44 = calcBruttoNetto(Object.assign({}, opt, {
    brutto: hi,
    steuerklasse: 4
  })).netto + calcBruttoNetto(Object.assign({}, opt, {
    brutto: lo,
    steuerklasse: 4
  })).netto;
  const n35 = calcBruttoNetto(Object.assign({}, opt, {
    brutto: hi,
    steuerklasse: 3
  })).netto + calcBruttoNetto(Object.assign({}, opt, {
    brutto: lo,
...
function _fmt(n) {
  return typeof fmtEUR === "function" ? fmtEUR(Math.round(n)) : Math.round(n).toLocaleString("de-DE") + " €";
}
function _pct(n) {
...
function _profile(ia, tweaks, yr, K) {
  ...
  const gst = brutto > 0 ? typeof _calcPreciseGST === "function" ? Math.round(_calcPreciseGST({
    brutto,
    _ia: ia
  }, yr) * 100) : K && typeof calcGrenzsteuersatz === "function" ? (() => {
    // K kommt von getK() — alle SV-Felder immer vorhanden
    const bbgKv = K.bbg_kv_monatlich * 12;
    const bbgRv = K.bbg_rv_monatlich * 12;
    const sv = Math.round(Math.min(brutto, bbgKv) * K.sv_kv + Math.min(brutto, bbgRv) * (K.sv_rv + K.sv_alv) + Math.min(brutto, bbgKv) * K.sv_pv);
  ...
  return {
    ...
    gst,
    _ia: ia,
    _tweaks: typeof tweaks !== 'undefined' ? tweaks : {}
  };
}
...
function _h_profil(p, yr, K, ia, tweaks, state, investments) {
  ...
  const bn = typeof calcBruttoNetto === "function" ? calcBruttoNetto({
    brutto: p.brutto,
    steuerklasse: p.sk,
    kirchensteuer: p.kstMit,
    bundesland: p.bl,
...
function _h_optimieren(p, yr, K, ia, tweaks, state, investments) {
  ...
  const sim = typeof simulateOptimalPlan === "function" ? simulateOptimalPlan(profile, valid, yr) : null;
...
function _h_werbungskosten(p, yr, K, state) {
  ...
  if (stRaw.length > 0) {
    ...
    pendLocs = [];
    if (p1km > 0) pendLocs.push({
      name: "Hauptstandort",
      km: p1km,
      tage: p1tage
    });
    if (p2km_ > 0) pendLocs.push({
      name: "Zweiter Standort",
      km: p2km_,
      tage: p2tage_
    });
...
function _h_kinder(p, yr, K) {
  const raw = window.TAX_CONFIG_RAW || {};
  const kg = raw.kindergeld_monatlich?.[`${yr >= 2026 ? "ab_2026" : "bis_2025"}`] || (yr >= 2026 ? 259 : 255);
...
function _detectModifiers(msg) {
  ...
  return {
    ...
    neu_job: /(erst|nur|seit).{0,20}(monat|woche|jahr).{0,20}(im job|beschäftigt|angestellt|im unternehmen|beim arbeitgeber|in der firma)/.test(m) || /(angefangen|begonnen|gestartet).{0,20}(arbeit|job|stelle)/.test(m),
...
{
  key: "afa",
  re: /afa|absetz.{0,10}abnutzung|abschreib|abschreibung|nutzungsdauer|lineare.*abschreib|degressive.*abschreib/
}, {
  key: "gwg",
  re: /gwg|geringwert|800.?euro.{0,15}(grenze|limit|abschreib|sofort)|sofort.{0,10}abschreib|sofortabschreib/
},
// Grundbegriffe
{
  key: "grenzsteuersatz",
  re: /grenzsteuersatz|grenz.?steuersatz|marginaler steuersatz|letzter euro|nächster euro.{0,20}steuer/
}, {
  key: "effektivsteuersatz",
  re: /effektiv.{0,10}steuersatz|durchschnittssteuersatz|durchschnittlicher steuersatz|real.{0,10}steuersatz/
}, {
  key: "progression",
  re: /progression|progressive.{0,10}steuer|steuerprogression|je mehr.{0,20}desto mehr.{0,20}steuer|steuersatz.{0,20}steigt/
}, {
  key: "zuflussprinzip",
  re: /zuflussprinzip|zufluss.{0,10}prinzip|wann.{0,20}zu versteuern|wann.{0,20}steuer.{0,20}fällig|wann gilt|in welchem jahr.{0,20}steuer/
}, {
  key: "pauschale_vs_real",
  re: /pauschale.{0,20}(vs|versus|oder|gegen).{0,20}(tatsächlich|real|nachweis|beleg)|was (bedeutet|ist|heißt).{0,20}pauschale|über.{0,10}pauschale|unter.{0,10}pauschale|pauschale überschreiten|pauschale unterschreiten|mehr als die pauschale|weniger als die pauschale/
},
// Freibeträge
{
  key: "grundfreibetrag_konzept",
  re: /grundfreibetrag|was ist steuerfrei|bis wann keine steuer|ab wann (zahle ich|muss ich).{0,20}steuer|steuerfreier betrag|existenzminimum/
}, {
  key: "sparer_pauschbetrag_konzept",
  re: /sparer.?pauschbetrag|sparer.?freibetrag|kapital.{0,15}freibetrag|1000.?euro.{0,15}kapital|2000.?euro.{0,15}kapital/
}, {
  key: "arbeitnehmer_pauschbetrag_konzept",
  re: /arbeitnehmer.?pausch(betrag|ale)|werbungskosten.{0,15}pausch|1230.?euro|automatisch.{0,20}werbungskosten|standard.{0,15}werbungskosten/
},
// Pendeln
{
  key: "pendlerpauschale_konzept",
  re: /pendlerpauschale.{0,20}(wie|was|gilt|funktioniert|berechnet|bedeutet)|was (ist|bedeutet) pendlerpauschale|entfernungspauschale.{0,20}(wie|was)/
},
// Kapital
{
  key: "abgeltungsteuer_konzept",
  re: /abgeltung(steuer|steur)|kapitalertrag.{0,20}(steuersatz|wie viel|was zahle|prozent)|25.{0,10}%.{0,20}kapital|flat.?tax|kapital.{0,20}pauschal.{0,20}versteuert/
}, {
  key: "freistellungsauftrag_konzept",
  re: /freistellungsauftrag|fsa|freistellung.{0,15}(einrichten|stellen|erteilen|bank)|kapital.{0,15}steuerfrei|bank.{0,15}freistellen/
}, {
  key: "verlustverrechnung_konzept",
  re: /verlustverrechnung.{0,20}(wie|was|funktioniert)|verlust.{0,10}topf|töpfe.{0,10}verlust|verluste.{0,20}verrechnen.{0,20}(wie|was|geht)/
}, {
  key: "spekulationsfrist_konzept",
  re: /spekulationsfrist|1.?jahr.{0,20}aktien.{0,20}steuerfrei|ein.?jährige.{0,10}frist|nach einem jahr.{0,20}steuer|jahresfrist.{0,20}aktien|wann sind aktien steuerfrei/
}, {
  key: "teilfreistellung_konzept",
  re: /teilfreistellung|etf.{0,20}(steuer|versteuert|wie viel)|30.?%.{0,20}(etf|fonds)|fondssteuer|wie wird etf.{0,20}versteuert/
}, {
  key: "vorabpauschale_konzept",
  re: /vorabpauschale|vorab.?pauschale|thesaurierend.{0,20}steuer|ausschüttungsgleich|basiszins.{0,20}etf/
}, {
  key: "guenstiger_pruefung_konzept",
  re: /günstigerprüfung.{0,20}(was|wie|wann)|günstigerwahlrecht.{0,20}(was|wie)|kapital.{0,20}persönlichem steuersatz|persönlicher satz.{0,20}kapital|niedriger.{0,20}steuersatz.{0,20}kapital/
},
// Altersvorsorge
{
  key: "ruerup_konzept",
  re: /rürup.{0,20}(wie|was|für wen|lohnt|funktioniert|mechanismus)|was ist rürup|basisrente.{0,20}(wie|was)/
}, {
  key: "riester_konzept",
  re: /riester.{0,20}(wie|was|funktioniert|lohnt|mechanismus|zulage)|was ist riester|riester.{0,20}förderung.{0,20}(wie|was)/
}, {
  key: "bav_konzept",
  re: /bav.{0,20}(wie|was|funktioniert)|betriebliche altersvorsorge.{0,20}(wie|was|funktioniert)|entgeltumwandlung.{0,20}(wie|was)|§\s*3.{0,5}nr.{0,5}63.{0,20}(wie|was)/
},
// Außergew. Belastungen
{
  key: "zumutbare_eigenbelastung",
  re: /zumutbare.{0,10}eigenbelastung|eigenbelastung.{0,20}(was|wie|wie viel|bedeutet)|selbstbehalt.{0,20}§.?33|warum (bekomme|kriege) ich nicht alles (erstattet|anerkannt).{0,20}arzt/
}, {
  key: "behinderten_pauschbetrag_konzept",
  re: /behinderten.?pauschbetrag.{0,20}(wie hoch|wie viel|was|wie)|gdb.{0,20}(pauschbetrag|steuer|wie viel)|schwerbehinderung.{0,20}(pauschbetrag|steuer|wie viel)/
},
// §35a
{
  key: "haushaltsnahe_konzept",
  re: /§.?35a|haushaltsnahe.{0,20}(was ist|wie|dienstleistungen.*was|systematik|wie funktioniert)|handwerker.{0,20}steuer.{0,20}(wie|was)|(putzhilfe|reinigung|gartenarbeit).{0,20}(wie|was).{0,20}steuer/
},
// Sonderausgaben
{
  key: "sonderausgaben_konzept",
  re: /sonderausgaben.{0,20}(was (sind|zählt|gehört|ist)|wie|systematik|unterschied zu werbungskosten)|unterschied.{0,20}(werbungskosten|sonderausgaben).{0,20}(sonderausgaben|werbungskosten)/
}, {
  key: "vorsorgeaufwendungen_konzept",
  re: /vorsorgeaufwendungen.{0,20}(was|wie)|krankenversicherung.{0,20}(absetzen|wie|was)|kv.{0,20}(steuer|absetzen)|pflichtbeiträge.{0,20}(steuer|absetzen)/
},
// Studentenstatus
{
  key: "student_steuer_konzept",
  re: /(student|studier).{0,30}(steuer|erststudium|zweitstudium|absetzen|werbungskosten|sonderausgaben)|erststudium.{0,20}(steuer|absetzen|warum nicht)|studiumskosten.{0,20}(steuer|wie)/
}, {
  key: "azubi_steuer_konzept",
  re: /(azubi|auszubildende|ausbildung).{0,30}(steuer|absetzen|werbungskosten|wie viel)/
},
// Sonstige wichtige Konzepte
{
  key: "progressionsvorbehalt",
  re: /progressionsvorbehalt|kurzarbeit.{0,20}steuer|elterngeld.{0,20}steuer|krankengeld.{0,20}steuer|warum.{0,20}(kurzarbeit|elterngeld|krankengeld).{0,20}(steuer|erhöht|mehr steuer)/
}, {
  key: "fuenftelregelung",
  re: /fünftelregel(ung)?|abfindung.{0,20}(steuer|wie|wie viel|berechnet)|außerordentliche einkünfte.{0,20}(steuer|wie)/
}, {
  key: "zusammenveranlagung_konzept",
  re: /zusammenveranlag.{0,20}(was|wie|lohnt|vorteil)|ehegattensplitting.{0,20}(was|wie|lohnt)|splitting(vorteil|tarif).{0,20}(was|wie)|gemeinsam.{0,20}veranlag.{0,20}(was|wie|lohnt)/
}, {
  key: "steuerklassen_konzept",
  re: /steuerklasse.{0,20}(was bedeutet|wie funktioniert|was ist|erkläre|was sind die unterschiede|welche gibt es)|welche steuerklasse.{0,20}(gibt es|existieren|bedeuten)/
}, {
  key: "steuerklasse_verheiratet",
  re: /steuerklasse.{0,20}(verheiratet|ehe|heirat|3.{0,3}5|4.{0,3}4|faktorverfahren)|verheiratet.{0,20}(welche steuerklasse|steuerklassenkombination|3\/5|4\/4)|(3.{0,3}5|4.{0,3}4).{0,20}steuerklasse.{0,20}(verheiratet|was (ist|bedeutet) besser)/
}, {
  key: "verlustvortrag_konzept",
  re: /verlustvortrag.{0,20}(was|wie|funktioniert)|verluste.{0,20}(vortragen|ins nächste jahr|in folgejahre|mehrere jahre).{0,20}(wie|was)|negative einkünfte.{0,20}(wie|was)/
}, {
  key: "festsetzungsverjaehrung_konzept",
  re: /festsetzungsverjährig|4.{0,5}jahre.{0,20}(rückwirkend|nachreichen|steuer)|wie weit.{0,20}rückwirkend|wie viele jahre.{0,20}(nachreichen|abgeben)/
}, {
  key: "einspruch_konzept",
  re: /einspruch.{0,20}(wie|was|frist|wann|gegen|einlegen)|widerspruch.{0,20}steuerbescheid|bescheid.{0,20}(anfechten|anzweifeln|nicht einverstanden|falsch)/
}, {
  key: "vorlaeufiger_bescheid",
  re: /vorläufig.{0,10}bescheid|bescheid.{0,20}vorläufig|was bedeutet vorläufig.{0,20}bescheid/
}, {
  key: "kinderfreibetrag_konzept",
  re: /kinderfreibetrag.{0,20}(vs|versus|oder|gegen|was ist besser).{0,20}kindergeld|kindergeld.{0,20}(vs|versus|oder|gegen|was ist besser).{0,20}kinderfreibetrag|günstigerprüfung.{0,20}kind/
}, {
  key: "ubungsleiter_freibetrag_konzept",
  re: /übungsleiter(freibetrag|pauschale)?|§.?3.{0,5}nr.{0,5}26|sporttrainer.{0,20}steuer|trainer.{0,20}steuerfrei|ehrenamt.{0,20}vergütung.{0,20}steuer/
}, {
  key: "inflationsausgleich_konzept",
  re: /inflationsausgleich(sprämie)?|3000.?euro.{0,20}(steuerfrei|arbeitgeber|prämie)|steuerfreie.{0,20}prämie.{0,20}arbeitgeber/
}, {
  key: "dienstwagen_konzept",
  re: /dienstwagen.{0,20}(steuer|wie|1%.{0,5}regel|fahrtenbuch|was)|1%.{0,5}regel.{0,20}dienstwagen|firmenwagen.{0,20}(steuer|versteuern|wie)/
}, {
  key: "doppelte_hh_konzept",
  re: /doppelte.{0,5}haushalt.{0,20}(voraussetzung|wann|was|wie)|zweitwohnung.{0,20}(absetzen|steuer|voraussetzung|was)|zweiter wohnsitz.{0,20}(beruf|arbeit|absetzen|was)/
}, {
  key: "verpflegungsmehraufwand_konzept",
  re: /verpflegungsmehraufwand.{0,20}(was|wie|wann)|vma|auswärtstätigkeit.{0,20}(essen|verpflegung|kosten|was)|dienstreise.{0,20}(essen|verpflegung|pauschale|was)/
}, {
  key: "photovoltaik_konzept",
  re: /photovoltaik.{0,20}(steuer|versteuern|steuerfrei|was|wie)|pv.{0,5}anlage.{0,20}(steuer|was)|einspeisevergütung.{0,20}(steuer|versteuern)/
}, {
  key: "minijob_konzept",
  re: /minijob.{0,20}(was|wie|unterschied|midi|midijob)|geringfügig.{0,20}beschäftigung.{0,20}(was|wie)|538.{0,20}grenze|unterschied.{0,20}(mini|midi).?job/
}];
...
function _kbTokens(yr, K) {
  const raw = window.TAX_CONFIG_RAW || {};
  const hoTag = K.homeoffice_pro_tag || 6,
    hoMax = K.homeoffice_max_tage || 210;
  return {
    jahr: yr,
...
const _WISSEN = {
  grundfreibetrag_konzept: {
    ...
    funktionsweise: ["Das zvE ist NICHT dein Brutto: zvE = Brutto − Sozialversicherung − Werbungskosten − Sonderausgaben − Freibeträge", "Der Freibetrag steckt direkt in der Tarifformel — du musst nichts beantragen", "Bei Zusammenveranlagung verdoppelt er sich für das Paar"],
  ...
  arbeitnehmer_pauschbetrag_konzept: {
    titel: "Arbeitnehmer-Pauschbetrag (Werbungskosten-Pauschale)",
    paragraph: "§ 9a S. 1 Nr. 1a EStG",
    ...
    funktionsweise: ["Das Finanzamt zieht {wkp} immer ab — auch bei 0 € echten Kosten", "Trägst du Einzelposten ein, gilt: max(Pauschale, Summe der Einzelposten)", "Nur der Betrag ÜBER der Pauschale bringt zusätzliche Ersparnis"],
  ...
  pendlerpauschale_konzept: {
    titel: "Pendlerpauschale (Entfernungspauschale)",
    ...
    funktionsweise: ["Formel: Arbeitstage × einfache km × Satz", "Homeoffice-Tage zählen nicht (kein Pendeln = kein Abzug, dafür Tagespauschale)", "Zu Fuß, Rad, Bahn, Auto, Mitfahrer — der Satz ist identisch"],
    ...
    fallstricke: ["Nicht Hin- UND Rückweg ansetzen — nur einfache Entfernung", "Kürzeste Straßenverbindung zählt; längere Route nur wenn verkehrsgünstiger und regelmäßig genutzt", "ÖPNV-Tickets über der Pauschale: tatsächliche Kosten ansetzbar"],
    fuer: {
  ...
  abgeltungsteuer_konzept: {
    ...
    funktionsweise: ["Bank zieht automatisch ab: 25 % + 5,5 % Soli darauf (effektiv 26,375 %)", "Vorher wird der Sparerpauschbetrag {spb} (verheiratet {spb2}) verrechnet — wenn ein Freistellungsauftrag vorliegt", "Liegt dein persönlicher Steuersatz unter 25 % → Günstigerprüfung beantragen"],
    ...
    fallstricke: ["Ohne Freistellungsauftrag besteuert die Bank ab dem ersten Euro — Rückholung nur über Anlage KAP", "Aktien-Verluste sind nur mit Aktien-Gewinnen verrechenbar (eigener Verlusttopf)"],
    fuer: {
  ...
  freistellungsauftrag_konzept: {
    titel: "Freistellungsauftrag (FSA)",
    paragraph: "§ 44a EStG",
    ...
    funktionsweise: ["Bei jeder Bank einzeln stellbar — die Summe aller FSAs darf {spb} nicht überschreiten", "Aufteilung frei wählbar (z.B. 600 € Broker, 400 € Tagesgeldbank)", "Änderung jederzeit online möglich"],
  ...
  guenstiger_pruefung_konzept: {
    ...
    funktionsweise: ["Kreuz in Anlage KAP, Zeile 4 — mehr nicht", "Lohnt rechnerisch ab persönlichem Grenzsteuersatz < 25 % (zvE grob unter ~19.000 €)", "Verlieren kannst du nichts: ist der Tarif schlechter, bleibt es bei 25 %"],
    ...
    fuer: {
      student: "Für Studenten mit Depot fast immer ein Gewinn — prüfen lassen kostet nichts."
    },
    verwandt: ["abgeltungsteuer_konzept"]
  },
  teilfreistellung_konzept: {
    titel: "Teilfreistellung bei Fonds/ETFs",
    paragraph: "§ 20 InvStG",
    ...
    funktionsweise: ["Gilt für Ausschüttungen, Vorabpauschale UND Verkaufsgewinne", "Mischfonds (≥ 25 % Aktien): 15 % frei · Immobilienfonds: 60/80 %", "Die Bank rechnet das automatisch — du siehst es in der Abrechnung"],
  ...
  vorabpauschale_konzept: {
    ...
    funktionsweise: ["Berechnung: Fondswert × 70 % des Basiszinses (BMF, jährlich) — gedeckelt auf die tatsächliche Wertsteigerung", "Abbuchung Anfang Januar fürs Vorjahr direkt vom Verrechnungskonto", "Beim späteren Verkauf werden gezahlte Vorabpauschalen angerechnet — keine Doppelbesteuerung"],
  ...
  riester_konzept: {
    ...
    funktionsweise: ["Volle Zulage bei Eigenbeitrag ≥ 4 % des Vorjahres-Bruttos (abzgl. Zulagen), min. 60 €", "Finanzamt macht automatisch die Günstigerprüfung: Zulage vs. Steuerersparnis aus {riesterMaxSA}-Abzug — du bekommst das Bessere", "Auszahlung im Alter voll steuerpflichtig (nachgelagerte Besteuerung)"],
    ...
    eintrag: "Anlage AV",
    fuer: {
      verheiratet: "Mittelbar förderberechtigte Ehepartner bekommen die Zulage schon ab 60 € Eigenbeitrag."
    },
    verwandt: ["ruerup_konzept", "bav_konzept"]
  },
  ruerup_konzept: {
    titel: "Rürup-/Basisrente",
    paragraph: "§ 10 Abs. 1 Nr. 2 EStG",
    ...
    funktionsweise: ["Jeder eingezahlte Euro senkt das zvE sofort — Ersparnis = Beitrag × Grenzsteuersatz", "Höchstbetrag teilt sich mit den Pflichtbeiträgen zur gesetzlichen RV", "Rente später steuerpflichtig (Besteuerungsanteil je Rentenbeginn)"],
    ...
    fuer: {
      selbstaendig: "Praktisch die einzige Vorsorgeform mit vollem Sofortabzug für dich — Kernbaustein der Steuerplanung."
    },
    verwandt: ["riester_konzept", "bav_konzept"]
  },
  bav_konzept: {
    titel: "Betriebliche Altersvorsorge (Entgeltumwandlung)",
    ...
    funktionsweise: ["100 € Umwandlung kosten dich netto oft nur ~50–55 €", "Arbeitgeberzuschuss von 15 % ist seit 2022 für alle Verträge Pflicht", "Auszahlung später steuer- und KV-pflichtig (Freibetrag in der KVdR beachten)"],
  ...
  zumutbare_eigenbelastung: {
    titel: "Zumutbare Eigenbelastung",
    ...
    funktionsweise: ["Stufenberechnung: bis 15.340 € / bis 51.130 € / darüber — je eigener Prozentsatz", "Mit Kindern sinken die Sätze deutlich (2–4 % statt 5–7 %)", "Deshalb: planbare Gesundheitskosten (Zahnersatz, Brille, OP) in EIN Jahr bündeln"],
    ...
    fuer: {
      verheiratet: "Bei Zusammenveranlagung zählt das gemeinsame Einkommen für die Schwelle — Sätze sind aber niedriger."
    },
    verwandt: ["haushaltsnahe_konzept"]
  },
  haushaltsnahe_konzept: {
    titel: "Haushaltsnahe Dienstleistungen & Handwerker",
    paragraph: "§ 35a EStG",
    ...
    funktionsweise: ["Nur ARBEITS-, Fahrt- und Maschinenkosten zählen — Material nie", "Rechnung + Überweisung Pflicht (Barzahlung = 0 €)", "Wirkt unabhängig vom Grenzsteuersatz — 20 % für jeden"],
  ...
  gwg: {
    titel: "Geringwertige Wirtschaftsgüter (GWG)",
    ...
    funktionsweise: ["Über der Grenze: Abschreibung über die AfA-Nutzungsdauer", "Ausnahme Computer/Software: seit 2021 IMMER Sofortabschreibung, egal wie teuer", "Berufliche Nutzung < 90 %? → anteilig ansetzen"],
  ...
  afa: {
    titel: "AfA — Absetzung für Abnutzung",
    paragraph: "§ 7 EStG",
    ...
    funktionsweise: ["Kaufpreis ÷ Nutzungsdauer = Jahresbetrag; im Kaufjahr monatsgenau", "AfA-Tabellen des BMF legen die Dauer fest", "Computer/Software: seit 2021 Nutzungsdauer 1 Jahr → faktisch Sofortabzug"],
  ...
  progressionsvorbehalt: {
    ...
    funktionsweise: ["Fiktive Rechnung: Steuersatz wird ermittelt, ALS OB die Leistung steuerpflichtig wäre", "Dieser höhere Satz trifft dann nur das tatsächlich steuerpflichtige Einkommen", "Folge: oft Nachzahlung + Pflicht zur Abgabe ab 410 € Leistungsbezug"],
  ...
  fuenftelregelung: {
    titel: "Fünftelregelung (Abfindungen)",
    paragraph: "§ 34 EStG",
    ...
    funktionsweise: ["Steuer auf (Einkommen + 1/5 Abfindung) minus Steuer auf Einkommen, Differenz × 5", "Seit 2025 nicht mehr automatisch im Lohnsteuerabzug — MUSS über die Steuererklärung beantragt werden", "Maximaler Effekt, wenn das übrige Einkommen im Abfindungsjahr NIEDRIG ist"],
  ...
  progression: {
    titel: "Steuerprogression & Grenzsteuersatz",
    paragraph: "§ 32a EStG",
    ...
    funktionsweise: ["Jeder abgesetzte Euro spart genau den Grenzsteuersatz — nicht den Durchschnittssatz", "Mythos „mehr Brutto lohnt nicht“: FALSCH — nur der Zuwachs wird höher besteuert, nie das Bestehende", "42 % greifen {jahr} ab ~69.900 € zvE, 45 % ab 277.826 €"],
  ...
  dienstwagen_konzept: {
    titel: "Dienstwagen-Besteuerung",
    paragraph: "§ 8 Abs. 2 EStG",
    ...
    funktionsweise: ["E-Autos: nur 0,25 % (Listenpreis ≤ 70.000 €), Hybride 0,5 %", "Fahrtenbuch lohnt bei wenig Privatnutzung oder altem/abgeschriebenem Fahrzeug", "Zuzahlungen des Arbeitnehmers mindern den Vorteil"],
...
function _kbAntwort(key, ft, mod, p, yr, K) {
  ...
  if (W.verwandt && W.verwandt.length) {
    const labels = W.verwandt.map(function (k) {
      return _WISSEN[k] ? _WISSEN[k].titel : k;
    }).slice(0, 3);
    teile.push("💬 *Frag mich auch zu:* " + labels.map(function (l) {
      return "*„" + l + "“*";
    }).join(" · "));
  }
  return teile.join("\n\n") + _TAG;
...
`;
    ...
    default:
      return null;
    // Kein Konzept gefunden → weitergeben
  }
}
...
function _dispatchIntent(intent, p, yr, K, ia, tweaks, state, investments) {
  switch (intent) {
    case "hilfe":
      return _h_hilfe();
    case "profil":
      return _h_profil(p, yr, K, ia, tweaks, state, investments);
    case "optimieren":
      return _h_optimieren(p, yr, K, ia, tweaks, state, investments);
    case "homeoffice":
      return _h_homeoffice(p, yr, K);
    case "pendler":
      return _h_pendler(p, yr, K);
    case "werbungskosten":
      return _h_werbungskosten(p, yr, K, state);
    case "bruttoNetto":
      return _h_bruttoNetto(p, yr, K);
    case "steuerklasse":
      return _h_steuerklasse(p, yr);
    case "grundfreibetrag":
      return _h_grundfreibetrag(yr, K);
    case "sonderausgaben":
      return _h_sonderausgaben(p, yr, K, ia);
    case "haushaltsnahe":
      return _h_haushaltsnahe(p, yr, K, state);
    case "kapital":
      return _h_kapital(p, yr, K, investments);
    case "guenstiger":
      return _h_guenstiger(p, yr, K, investments);
    case "soli":
      return _h_soli(yr, K);
    case "kirchensteuer":
      return _h_kirchensteuer(p, yr);
    case "fristen":
      return _h_fristen(yr);
    case "minijob":
      return _h_minijob(yr, K);
    case "riester":
      return _h_riester(p, yr, K, ia);
    case "ruerup":
      return _h_ruerup(p, yr);
    case "bav":
      return _h_bav(p, yr, K, ia);
    case "aussergewoehnlich":
      return _h_aussergewoehnlich(p, yr, K);
    case "studenten":
      return _h_studenten(yr, K);
    case "verlust":
      return _h_verlust(yr);
    case "arbeitsmittel":
      return _h_arbeitsmittel(p, yr);
    case "doppelteHH":
      return _h_doppelteHH(p, yr, K);
    case "unterhalt":
      return _h_unterhalt(p, yr, K);
    case "kinder":
      return _h_kinder(p, yr, K);
    case "rueckwirkend":
      return _h_rueckwirkend(yr);
    case "elster":
      return _h_elster(yr);
    default:
      return _h_default(p, yr);
  }
}

// ── Haupt-Dispatcher v3: Slots + Dialog-Gedächtnis + Fuzzy + Szenarien ──────
function deterministicSteuerAntwort(userMsg, {
  tweaks = {},
  state = {},
  investments = {},
  interviewAnswers = {}
  ...
  if (_NLU_CTX.pendingChoice && Date.now() - _NLU_CTX.ts < 10 * 60 * 1000) {
    const c1 = _NLU_CTX.pendingChoice[0],
      c2 = _NLU_CTX.pendingChoice[1];
    const low = String(userMsg).toLowerCase().trim();
    let gewaehlt = null;
    if (/^(1\b|eins\b|erste|ersteres|ja\b)/.test(low)) gewaehlt = c1;else if (/^(2\b|zweite|zweiteres)/.test(low)) gewaehlt = c2;else {
      const l1 = (_INTENT_LABELS[c1] || c1).toLowerCase().slice(0, 7);
      const l2 = (_INTENT_LABELS[c2] || c2).toLowerCase().slice(0, 7);
      if (low.includes(l1)) gewaehlt = c1;else if (low.includes(l2)) gewaehlt = c2;
    }
    ...
    if (!gewaehlt) {
      const passt = function (intent) {
        if (intent === "bruttoNetto") return slotsNew.brutto != null || slotsNew._nums.some(function (v) {
          return v >= 3000 && v <= 2000000;
        });
      ...
      const p1 = passt(c1),
        p2 = passt(c2);
      if (p1 && !p2) gewaehlt = c1;else if (p2 && !p1) gewaehlt = c2;
      if (gewaehlt === "bruttoNetto" && slotsNew.brutto == null) {
        const cand = slotsNew._nums.filter(function (v) {
          return v >= 3000 && v <= 2000000;
        });
        if (cand.length) {
          slotsNew.brutto = cand[0];
          slotsNew.bruttoList = cand;
        }
      }
    }
    _NLU_CTX.pendingChoice = null;
    if (gewaehlt) {
      const mergedC = Object.assign({}, _NLU_CTX.slots || {}, slotsNew, {
        _nums: slotsNew._nums
      });
  ...
  if (_NLU_CTX.pendingSlot && Date.now() - _NLU_CTX.ts < 10 * 60 * 1000) {
    ...
    if (hatEigenesThema) {
      ...
      if (ps.slot === "km" && slotsNew.km == null) {
        const c = slotsNew._nums.filter(function (v) {
          return v >= 1 && v <= 400;
        });
        if (c.length) {
          slotsNew.km = c[0];
      ...
      if (ps.slot === "brutto" && slotsNew.brutto == null) {
        ...
        if (c.length) {
          slotsNew.brutto = c[0];
          slotsNew.bruttoList = c;
        }
      }
      const gefuellt = ps.slot === "km" && slotsNew.km != null || ps.slot === "ho_tage" && (slotsNew.ho_tage != null || slotsNew.tage_pro_woche != null) || ps.slot === "brutto" && slotsNew.brutto != null;
      if (gefuellt) {
        const mergedS = Object.assign({}, _NLU_CTX.slots || {}, slotsNew, {
          _nums: slotsNew._nums
        });
  ...
  if (!followUpVon && _NLU_CTX.intent && Date.now() - _NLU_CTX.ts < 30 * 60 * 1000) {
    ...
    if (mShort.length <= 70 && _FOLLOWUP_RE.test(mShort) && nurParamIntents && (slotsNew.sk != null || slotsNew.kinder != null) && calcThemen.indexOf(_NLU_CTX.intent) !== -1) {
  ...
  if (intents[0] === "default") {
    ...
    if (st.best && st.sim >= 0.34 && st.sim - st.sim2 >= 0.05) {
      ...
      _NLU_CTX = {
        intent: _NLU_CTX.intent,
        slots: Object.assign({}, _NLU_CTX.slots || {}, slotsNew),
        ts: Date.now(),
        pendingChoice: [st.best, st.second],
        pendingSlot: null
      };
  ...
  const slots = followUpVon ? Object.assign({}, _NLU_CTX.slots || {}, slotsNew, {
    _nums: slotsNew._nums
  }) : slotsNew;
  ...
  if (konzeptAntwort) {
    _NLU_CTX = {
      intent: intents[0] !== "default" ? intents[0] : _NLU_CTX.intent,
      slots,
      ts: Date.now()
    };
  ...
  const fragtWo = ft === "wo_eintragen" || /wo\s+(trage|gebe|fülle|trägt)\b[\s\S]{0,60}\bein|welche[rs]?\s+(anlage|zeile|formular)|in\s+welche[rms]?\s+(anlage|zeile|formular)/i.test(userMsg);
  if (fragtWo) {
  ...
  if (intents.length === 1) {
    ...
    const parts = filtered.map(intent => _dispatchIntent(intent, p, yr, K, ia, tweaks, state, investments).replace(_TAG, "").trimEnd());
...
function ChatListItem({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bot-chat-item-main"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-chat-item-title"
  }, chat.title || "Neuer Chat"), /*#__PURE__*/React.createElement("div", {
    className: "bot-chat-item-sub"
  }, dateStr, " \xB7 ", (chat.messages || []).length, " Nachrichten")), /*#__PURE__*/React.createElement("button", {
    className: "bot-chat-item-del",
    onClick: e => {
      e.stopPropagation();
      onDelete();
    },
    title: "Chat l\xF6schen",
    "aria-label": "Chat l\xF6schen"
...
function _renderMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*\|/.test(line)) {
      ...
      }, React.createElement("thead", null, React.createElement("tr", null, headerRow.map((h, ci) => React.createElement("th", {
        key: ci,
        style: {
          padding: "5px 10px",
          textAlign: "left",
      ...
      }, inlineRender(h))))), React.createElement("tbody", null, bodyRows.map((row, ri) => React.createElement("tr", {
        key: ri,
        style: {
          borderBottom: "1px solid var(--border)"
        }
      }, parseRow(row).map((cell, ci) => React.createElement("td", {
        key: ci,
        style: {
          padding: "5px 10px",
          verticalAlign: "top",
    ...
    if (/^- /.test(line)) {
      ...
      }, items.map((item, ii) => React.createElement("li", {
        key: ii,
        style: {
          marginBottom: 2
        }
...
function MessageBubble({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `bot-msg bot-msg-${role}`
  }, /*#__PURE__*/React.createElement("div", {
...
function TypingIndicator() {
  return /*#__PURE__*/React.createElement("div", {
    className: "bot-msg bot-msg-assistant"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-msg-bubble bot-msg-typing"
  }, /*#__PURE__*/React.createElement("span", null), /*#__PURE__*/React.createElement("span", null), /*#__PURE__*/React.createElement("span", null)));
}
function SuggestionChips({
  onPick
}) {
  const suggestions = ["Lohnt sich meine Steuererklärung?", "Kann ich meinen Laptop absetzen?", "Wie funktioniert die Homeoffice-Pauschale?", "Was ist der Unterschied zwischen Erst- und Zweitstudium?"];
  return /*#__PURE__*/React.createElement("div", {
    className: "bot-suggestions"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-suggestions-label"
  }, "Vorschl\xE4ge zum Start"), /*#__PURE__*/React.createElement("div", {
    className: "bot-suggestions-chips"
...
function BotSettingsPanel({
  ...
  const testOllama = async () => {
    ...
    if (typeof window.ollamaTest === "function") {
      ...
      setTestStatus(ok ? {
        ok: true,
        msg: `✓ Ollama antwortet (${url}, Modell: ${model})`
      } : {
        ok: false,
        msg: `Ollama nicht erreichbar — läuft "ollama serve"? Modell "${model}" geladen?`
      });
    } else {
      setTestStatus({
        ok: false,
        msg: "ollamaTest nicht verfügbar — utils.jsx laden?"
      });
    }
    setTesting(false);
  };
  const RECOMMENDED = [{
    ...
    label: "llama3.1:8b",
    badge: "⭐ Empfohlen",
    desc: "Beste Balance auf M5 · Deutsch + Englisch · ~5 GB"
  }, {
    id: "mistral:7b",
    label: "mistral:7b",
    badge: "⚡ Schnell",
    desc: "Sehr schnelle Antworten · gutes Deutsch · ~4 GB"
  }, {
    id: "gemma3:12b",
    label: "gemma3:12b",
    badge: "🧠 Präzise",
    desc: "Beste Qualität · langsamer · ~8 GB RAM nötig"
  }, {
    id: "qwen2.5:7b",
    label: "qwen2.5:7b",
    badge: "🌐 Mehrsprachig",
    desc: "Sehr gutes Deutsch · Steuerbegriffe stark · ~4 GB"
  }];
  return /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
    ...
    className: "bot-header-title"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-main"
  }, "Backend"), /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-sub"
  }, "SteuerBot konfigurieren"))), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: "auto",
  ...
  }, "\xB7 kostenlos \xB7 lokal \xB7 kein API-Key")), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-muted)",
      marginBottom: 4,
  ...
  }, "Ollama-URL ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: "var(--text-faint)"
    }
  }, "(Standard: http://localhost:11434)")), /*#__PURE__*/React.createElement("input", {
    type: "text",
    value: ollamaUrl,
  ...
  }, "\uD83D\uDCF1 iPhone im Heimnetz: URL = IP deines Mac, z.B.", " ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface-2)",
      padding: "1px 5px",
      borderRadius: 4
    }
  }, "http://192.168.1.100:11434"), " ", "\u2014 Ollama mit", " ", /*#__PURE__*/React.createElement("code", {
    style: {
      background: "var(--surface-2)",
      padding: "1px 5px",
      borderRadius: 4
    }
  }, "OLLAMA_HOST=0.0.0.0 ollama serve"), " ", "starten."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-muted)",
      marginBottom: 4
    }
  }, "Modell-Name"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      alignItems: "center",
  ...
  }, testing ? "…" : "Test")), testStatus && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10,
      fontSize: 12,
      padding: "6px 10px",
  ...
  }, testStatus.msg), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-muted)",
      marginBottom: 6,
  ...
  }, "Empfohlen f\xFCr MacBook Air M5:"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 5
    }
  }, RECOMMENDED.map(m => /*#__PURE__*/React.createElement("button", {
    key: m.id,
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\uD83D\uDCD6 Ollama installieren (Anleitung)"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: "var(--text-muted)"
    }
  }, guideOpen ? "▲" : "▼")), guideOpen && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "12px 14px",
      fontSize: 12,
      lineHeight: 1.7,
  ...
  }, "Mac (M5):"), /*#__PURE__*/React.createElement("ol", {
    style: {
      margin: 0,
      paddingLeft: 18
    }
  }, /*#__PURE__*/React.createElement("li", null, "Besuche ", /*#__PURE__*/React.createElement("a", {
  ...
  }, "ollama.ai"), " \u2192 Download \u2192 macOS .dmg installieren"), /*#__PURE__*/React.createElement("li", null, "Terminal \xF6ffnen, Modell laden:", /*#__PURE__*/React.createElement("code", {
    style: {
      display: "block",
      background: "var(--surface)",
      padding: "4px 8px",
  ...
  }, "ollama pull llama3.1:8b")), /*#__PURE__*/React.createElement("li", null, "Ollama l\xE4uft automatisch im Hintergrund. URL: ", /*#__PURE__*/React.createElement("code", null, "http://localhost:11434"))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      margin: "10px 0 4px",
      color: "var(--text)"
    }
  }, "iPhone (Heimnetz):"), /*#__PURE__*/React.createElement("ol", {
    style: {
      margin: 0,
      paddingLeft: 18
    }
  }, /*#__PURE__*/React.createElement("li", null, "Mac-IP herausfinden: ", /*#__PURE__*/React.createElement("code", null, "ifconfig | grep \"inet 192\"")), /*#__PURE__*/React.createElement("li", null, "Ollama mit Netzwerkzugriff starten:", /*#__PURE__*/React.createElement("code", {
    style: {
      display: "block",
      background: "var(--surface)",
      padding: "4px 8px",
  ...
  }, "OLLAMA_HOST=0.0.0.0 ollama serve")), /*#__PURE__*/React.createElement("li", null, "URL oben eintragen: ", /*#__PURE__*/React.createElement("code", null, "http://192.168.x.x:11434"), " (deine Mac-IP)"), /*#__PURE__*/React.createElement("li", null, "iPhone und Mac m\xFCssen im gleichen WLAN sein")), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 10,
      padding: "6px 10px",
      borderRadius: 8,
  ...
  }, "\u26A0\uFE0F Kein Internet n\xF6tig \u2014 alles l\xE4uft lokal. Keine Daten verlassen dein Ger\xE4t."))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "12px 14px",
      borderRadius: 12,
      background: "oklch(0.95 0.03 145)",
      border: "1px solid oklch(0.82 0.07 145)",
      fontSize: 13,
      color: "oklch(0.34 0.10 145)",
      lineHeight: 1.55,
      marginBottom: 20
    }
  }, /*#__PURE__*/React.createElement("strong", null, "\u26A1 Offline-Modus immer aktiv"), /*#__PURE__*/React.createElement("br", null), "Wenn Ollama nicht erreichbar ist, antwortet der SteuerBot direkt aus der Steuer-Engine \u2014 28 Themenbereiche, echte Berechnungen, kein Backend n\xF6tig. Funktioniert auch komplett offline auf dem Handy."), /*#__PURE__*/React.createElement("button", {
    onClick: save,
    style: {
      width: "100%",
      padding: "11px",
  ...
  }, "Keine Daten verlassen das Ger\xE4t. Kein API-Key, kein Proxy, kein Cloud-Dienst.")));
}

// NEU: Feature B — interviewAnswers als Prop für personalisierten Kontext
function SteuerBotModal({
  open,
  onClose,
  tweaks,
  state,
  ...
  async function deleteChat(id) {
    ...
    setChats(next);
    if (activeId === id) {
      if (next.length > 0) selectChat(next[0].id);else newChat();
    }
  }
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      if (cfg.model && typeof window.ollamaStream === "function") {
        try {
          reply = await window.ollamaStream(cfg.url, cfg.model, system, apiMessages, token => {
          ...
          reply = deterministicSteuerAntwort(lastMsg, {
            tweaks,
            state,
            investments,
            interviewAnswers
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bot-header-title-main"
  }, "SteuerBot"), /*#__PURE__*/React.createElement("div", {
    className: "bot-header-title-sub"
  }, "Fragen zur deutschen Einkommensteuer")), /*#__PURE__*/React.createElement("button", {
    className: "bot-header-settings",
    onClick: () => setSettingsOpen(v => !v),
    title: "KI-Backend konfigurieren",
    ...
    className: "bot-header-close",
    onClick: onClose,
    "aria-label": "Schlie\xDFen"
  }, /*#__PURE__*/React.createElement(Icon.Close, null))), sidebarOpen && /*#__PURE__*/React.createElement("div", {
    className: "bot-sidebar",
    onClick: () => setSidebarOpen(false)
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-sidebar-inner",
    onClick: e => e.stopPropagation()
  }, /*#__PURE__*/React.createElement("div", {
    className: "bot-sidebar-header"
  }, /*#__PURE__*/React.createElement("span", null, "Gespeicherte Chats"), /*#__PURE__*/React.createElement("button", {
    className: "bot-sidebar-new",
    onClick: newChat
  }, "+ Neu")), chats.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "bot-sidebar-empty"
  }, "Noch keine gespeicherten Chats.") : /*#__PURE__*/React.createElement("div", {
    className: "bot-chat-list"
  }, chats.map(c => /*#__PURE__*/React.createElement(ChatListItem, {
    key: c.id,
    chat: c,
    active: c.id === activeId,
    onSelect: () => selectChat(c.id),
    onDelete: () => deleteChat(c.id)
  }))))), settingsOpen && /*#__PURE__*/React.createElement(BotSettingsPanel, {
    backend: backend,
    onClose: () => {
    ...
    className: "bot-empty-emoji"
  }, "\u2696\uFE0F"), /*#__PURE__*/React.createElement("div", {
    className: "bot-empty-title"
  }, "SteuerBot DE"), /*#__PURE__*/React.createElement("div", {
    className: "bot-empty-sub"
  }, "Frag mich zu Werbungskosten, Homeoffice, Pendler\xADpauschale, ETFs, Steuererkl\xE4rung oder Studenten-Sonderregeln. Ich kenne dein Steuer-Profil und deine erfassten Belege."), /*#__PURE__*/React.createElement(SuggestionChips, {
    onPick: sendMessage
  })) : /*#__PURE__*/React.createElement("div", {
    className: "bot-msgs"
    ...
    role: m.role,
    content: m.content
  })), busy && /*#__PURE__*/React.createElement(TypingIndicator, null)), error ? /*#__PURE__*/React.createElement("div", {
    className: "bot-error"
  }, /*#__PURE__*/React.createElement("strong", null, "Fehler:"), " ", error) : null), /*#__PURE__*/React.createElement("form", {
    ...
    disabled: busy
  }), draft.length > MAX_INPUT_CHARS * 0.85 && /*#__PURE__*/React.createElement("div", {
    className: "bot-input-counter",
    style: {
      position: "absolute",
      bottom: 54,
      right: 56,
  ...
  }, draft.length, "/", MAX_INPUT_CHARS), /*#__PURE__*/React.createElement("button", {
    type: "submit",
    className: "bot-send",
...
(function _secureExport() {
  try {
    Object.defineProperty(window, "SteuerBotModal", {
      value: SteuerBotModal,
      writable: false,
      configurable: false,
      enumerable: true
    });
  } catch {
    window.SteuerBotModal = SteuerBotModal;
  }
})();
